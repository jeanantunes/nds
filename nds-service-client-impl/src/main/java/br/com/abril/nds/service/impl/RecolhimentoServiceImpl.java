...
public class RecolhimentoServiceImpl implements RecolhimentoService {
	...
	private TreeMap<Date, List<ProdutoRecolhimentoDTO>> copiarMatriz(
			TreeMap<Date, List<ProdutoRecolhimentoDTO>> matrizOriginal) {
	...
	public BalanceamentoRecolhimentoDTO obterMatrizBalanceamento(Integer anoNumeroSemana,
		...
		RecolhimentoDTO dadosRecolhimento = this.obterDadosRecolhimento(anoNumeroSemana, listaIdsFornecedores,
				tipoBalanceamentoRecolhimento, forcarBalanceamento);

		BalanceamentoRecolhimentoStrategy balanceamentoRecolhimentoStrategy = BalanceamentoRecolhimentoFactory
				.getStrategy(tipoBalanceamentoRecolhimento);

		BalanceamentoRecolhimentoDTO balanceamentoRecolhimentoDTO = balanceamentoRecolhimentoStrategy
				.balancear(dadosRecolhimento);

		if (balanceamentoRecolhimentoDTO != null && balanceamentoRecolhimentoDTO.getMatrizRecolhimento() != null) {

			TreeMap<Date, List<ProdutoRecolhimentoDTO>> copiaMatriz = copiarMatriz(
					balanceamentoRecolhimentoDTO.getMatrizRecolhimento());

			Date dataMatriz;
			List<ProdutoRecolhimentoDTO> listaProdutos;

			for (Entry<Date, List<ProdutoRecolhimentoDTO>> entry : balanceamentoRecolhimentoDTO.getMatrizRecolhimento()
					.entrySet()) {
				...
				if (listaProdutos != null && !listaProdutos.isEmpty()) {
					...
					for (ProdutoRecolhimentoDTO prDTO : listaProdutos) {
						...
						/*
						 * if(prDTO!=null && dataMatriz!=null &&
						 * prDTO.getDataRecolhimentoPrevista().after(dataMatriz)
						 * && prDTO.getStatusLancamento().compareTo(
						 * StatusLancamento.EXPEDIDO)==0 ){ //remove e insere no
						 * dia certo
						 * 
						 * if(copiaMatriz.containsKey(prDTO.
						 * getDataRecolhimentoPrevista())){
						 * 
						 * copiaMatriz.get(dataMatriz).remove(prDTO);
						 * prDTO.setNovaData(prDTO.getDataRecolhimentoPrevista()
						 * );
						 * copiaMatriz.get(prDTO.getDataRecolhimentoPrevista()).
						 * add(prDTO); } }
						 */
	...
	public void salvarBalanceamentoRecolhimento(Usuario usuario,
			BalanceamentoRecolhimentoDTO balanceamentoRecolhimentoDTO, StatusLancamento statusLancamento,
			Date dataPesquisa) {

		Map<Date, List<ProdutoRecolhimentoDTO>> matrizRecolhimento = balanceamentoRecolhimentoDTO
			...
			for (ProdutoRecolhimentoDTO produtoRecolhimento : listaProdutoRecolhimentoDTO) {
				...
				if (!produtoRecolhimento.getIdsLancamentosAgrupados().isEmpty()) {
					...
					for (Long idLancamento : produtoRecolhimento.getIdsLancamentosAgrupados()) {
						...
						this.montarInformacoesSalvarBalanceamento(mapaRecolhimentos, idsLancamento,
								produtoRecolhimentoAgrupado);
					}
				}
				// }
			}
	...
	private void montarInformacoesSalvarBalanceamento(Map<Long, ProdutoRecolhimentoDTO> mapaRecolhimentos,
			Set<Long> idsLancamento, ProdutoRecolhimentoDTO produtoRecolhimento) {
	...
	public TreeMap<Date, List<ProdutoRecolhimentoDTO>> confirmarBalanceamentoRecolhimento(
		...
		for (Date dataConfirmada : datasConfirmadas) {
			...
			for (ProdutoRecolhimentoDTO produtoRecolhimento : produtosRecolhimento) {
				...
				if (produtoRecolhimento.isBalanceamentoConfirmado()) {
					...
					this.montarMatrizRecolhimentosConfirmados(matrizConfirmada, produtoRecolhimento, null,
							novaDataRecolhimento);

					continue;
				}

				this.montarInformacoesConfirmarBalanceamento(mapaLancamentoRecolhimento,
						mapaDataRecolhimentoLancamentos, idsLancamento, produtoRecolhimento, novaDataRecolhimento);
				...
				if (!idsLancamentosAgrupados.isEmpty()) {
					...
					for (Long idLancamento : idsLancamentosAgrupados) {
						...
						ProdutoRecolhimentoDTO produtoRecolhimentoAgrupado = this
								.obterProdutoRecolhimento(produtosRecolhimentoAgrupados, idLancamento);

						produtoRecolhimentoAgrupado.setNovaData(novaDataRecolhimento);

						this.montarInformacoesConfirmarBalanceamento(mapaLancamentoRecolhimento,
								mapaDataRecolhimentoLancamentos, idsLancamento, produtoRecolhimentoAgrupado,
								novaDataRecolhimento);
					}
				}
			}
		...
		this.atualizarLancamentos(idsLancamento, usuario, mapaLancamentoRecolhimento,
	...
	private void montarInformacoesConfirmarBalanceamento(Map<Long, ProdutoRecolhimentoDTO> mapaLancamentoRecolhimento,
			Map<Date, Set<Long>> mapaDataRecolhimentoLancamentos, Set<Long> idsLancamento,
			ProdutoRecolhimentoDTO produtoRecolhimento, Date novaDataRecolhimento) {
	 ...
	 * @param usuario
	 *            - usuário
	 * @param mapaLancamentoRecolhimento
	...
	private void atualizarLancamentos(Set<Long> idsLancamento, Usuario usuario,
			Map<Long, ProdutoRecolhimentoDTO> mapaLancamentoRecolhimento, StatusLancamento statusLancamento,
	 ...
	 * 
	 * @param matrizConfirmada
	 *            - matriz de recolhimento confirmada
	 * @param produtoRecolhimento
	 *            - produto de recolhimento
	 * @param lancamento
	 *            - lançamento
	 * @param novaData
	...
	private void montarMatrizRecolhimentosConfirmados(TreeMap<Date, List<ProdutoRecolhimentoDTO>> matrizConfirmada,
			ProdutoRecolhimentoDTO produtoRecolhimento, Lancamento lancamento, Date novaData) {
	 ...
	 */
	private List<CotaReparteDTO> processaListaCotaReparteDTOLancamento(List<CotaReparteDTO> cotasReparte,
			Long idLancamento) {
	 ...
	 */
	private List<ChamadaEncalhe> processaListaChamadaEncaleProdutoEdicao(List<ChamadaEncalhe> listaChamadaEncalhe,
	 ...
	 * 
	 * @param mapaDataRecolhimentoLancamentos
	 *            - mapa de datas de recolhimento e identificadores de
	 *            lancamentos.
	 * @param numeroSemana
	 *            - número da semana
	 * @param usuario
	...
	private void gerarChamadasEncalhe(Map<Date, Set<Long>> mapaDataRecolhimentoLancamentos, Integer numeroSemana,
		...
		//	Set<Long> ids = new TreeSet<Long>(lancamentoRepository.getIDByDataRecolhimento(datasConfirmadas));
			...
			for (Long idLancamento : idsLancamento) {
				...
				List<CotaReparteDTO> cotasReparteLancamento = this.processaListaCotaReparteDTOLancamento(cotasReparte,
						idLancamento);

				// odemir CE parciais para cota com reparte zero no periodo
				// final
				// ver se 'e produto parcial, se periodo do lancamento 'e final
				// entao buscar cotas que nao teve reparte neste lancamento mas
				// teve nos anteriores
				...
				if (produtoEdicao.isParcial() && lancamento.getPeriodoLancamentoParcial() != null
					...
					// buscar cotas que nao teve reparte neste lancamento mas
					// teve nos anteriores
					List<CotaReparteDTO> cotasRepartePeriodosAnteriores = this.movimentoEstoqueCotaRepository
							.obterRepartePeriodosAnteriores(lancamento);

					LOGGER.warn("QTDE DE COTAS SEM REPARTE NA FINAL MAS COM REPARTES ANTERIORES="
							+ cotasRepartePeriodosAnteriores.size());

					// incluir em cotasReparteLancamento
					for (CotaReparteDTO cotaRepartePeriodosAnteriores : cotasRepartePeriodosAnteriores) {
						CotaReparteDTO cr1 = null;
						int i = cotasReparteLancamento.indexOf(cotaRepartePeriodosAnteriores);
						if (i > 0)
							cr1 = cotasReparteLancamento.get(i);
						if (cr1 != null)
							LOGGER.warn("COTA " + cotaRepartePeriodosAnteriores.getCota().getId()
									+ " ja tem em reparte na chamada final com qtd=" + cr1.getReparte());
						if (!cotasReparteLancamento.contains(cotaRepartePeriodosAnteriores)
								&& cotaRepartePeriodosAnteriores.getReparte().intValue() > 0) {

							LOGGER.warn("PARCIAL FINAL COM REPARTE 0.. INSERINDO COTA="
									+ cotaRepartePeriodosAnteriores.getCota().getId() + "PRODUTOEDICAo="
									+ produtoEdicao.getId() + " qtde="
				...
				Set<ChamadaEncalhe> chamadasEncalheProdutoEdicao = new HashSet<>(
						this.processaListaChamadaEncaleProdutoEdicao(listaChamadaEncalhe, produtoEdicao.getId()));
				...
				for (CotaReparteDTO cotaReparte : cotasReparteLancamento) {
					...
					ChamadaEncalhe chamadaEncalhe = this.getChamadaEncalheMatrizRecolhimento(
							new ArrayList<>(chamadasEncalheProdutoEdicao), dataRecolhimento);
	...
	private List<ProdutoEdicaoECota> getChamadao(Set<Long> idsLancamento, Date dataOperacao) {
		return chamadaEncalheCotaRepository.obterListaChamadaoByListLancamento(idsLancamento, dataOperacao);
	}

	private void removerChamadaEncalheCotaAntecipadaChamadao(Cota cota, List<ChamadaEncalhe> chamadasEncalhe) {

		ChamadaEncalheCota chamadaEncalheCota = this.getChamadaEncalheCotaAntecipadaChamadao(chamadasEncalhe,
				cota.getId());
	...
	/**
	 * Verifica se a cota devolve encalhe para a criação ou não de
	 * ChamadaEncalheCota
	 * 
	 * @param tipoCota
	 ...
	 * @return boolean
	 */
	private boolean isDevolveEncalhe(final TipoCota tipoCota, Boolean devolveEncalhe) {
	 ...
	 * @param usuario
	 */
	private void criarChamadaEncalheCota(BigInteger qtdPrevista, Cota cota, ChamadaEncalhe chamadaEncalhe,
			Date dataLctoDistribuidor, boolean cotaContribuinteExigeNF, Usuario usuario, boolean isParcialFinal) {
		...
		if (BigInteger.ZERO.compareTo(qtdPrevista) >= 0) {
			if (isParcialFinal) { // se for pacialFinal, criar chamada com
									// zero..
				LOGGER.warn("CRIANDO CHAMADA ENCALHE COM ZERO PARA COTA=" + cota.getId() + "  chamada encalhe="
						+ chamadaEncalhe.getId());
				// return;
			} else {
				return;
			}
		...
		BigInteger qtdPrevistaExistente = chamadaEncalheCota.getQtdePrevista() != null
				? chamadaEncalheCota.getQtdePrevista() : BigInteger.ZERO;

		if (!qtdPrevistaExistente.equals(BigInteger.ZERO)
				&& qtdPrevistaExistente.intValue() == qtdPrevista.intValue()) {
			LOGGER.warn("ATENCAO.INCREMENTANDO QUANTIDADE PREVISTA NA CHAMADA ENCALHE COTA ..COTA.id=" + cota.getId()
					+ " chamada encalhe id =" + chamadaEncalhe.getId() + "  QTDE PREVISTA EXISTENTE="
					+ qtdPrevistaExistente + " QTDE PREVISTA A ADICIONAR=" + qtdPrevista.intValue());
		}
	...
	private ChamadaEncalheCota getChamadaEncalheCotaAntecipadaChamadao(List<ChamadaEncalhe> chamadasEncalhe,
			Long idCota) {
	 ...
	 */
	private RecolhimentoDTO obterDadosRecolhimento(Integer anoNumeroSemana, List<Long> listaIdsFornecedores,
			TipoBalanceamentoRecolhimento tipoBalanceamento, boolean forcarBalanceamento) {

		RecolhimentoDTO dadosRecolhimento = new RecolhimentoDTO();

		Intervalo<Date> periodoRecolhimento = getPeriodoRecolhimento(anoNumeroSemana);

		TreeSet<Date> datasRecolhimentoFornecedor = this.obterDatasRecolhimentoFornecedor(periodoRecolhimento,
				listaIdsFornecedores);

		dadosRecolhimento.setDatasRecolhimentoFornecedor(datasRecolhimentoFornecedor);

		TreeSet<Date> datasRecolhimentoDisponiveis = this
				.obterDatasRecolhimentoDisponiveis(datasRecolhimentoFornecedor);
		...
		if (TipoBalanceamentoRecolhimento.EDITOR.equals(tipoBalanceamento)) {
			...
			produtosRecolhimento = this.lancamentoRepository.obterBalanceamentoRecolhimentoPorEditorData(
			...
			produtosRecolhimento = this.lancamentoRepository.obterBalanceamentoRecolhimento(periodoRecolhimento,
					listaIdsFornecedores, GrupoProduto.CROMO);
		}

		TreeMap<Date, BigDecimal> mapaExpectativaEncalheTotalDiaria = this.lancamentoRepository
				.obterExpectativasEncalhePorData(periodoRecolhimento, listaIdsFornecedores, GrupoProduto.CROMO);
		...
		dadosRecolhimento.addMediaRecolhimentoDistribuidor(
				media.longValue() / dadosRecolhimento.getDatasRecolhimentoFornecedor().size());

		List<ProdutoRecolhimentoDTO> produtosRecolhimentoAgrupados = new ArrayList<>();

		produtosRecolhimento = this.tratarAgrupamentoLancamentosComMesmaEdicao(produtosRecolhimento,
				produtosRecolhimentoAgrupados);
		...
		dadosRecolhimento
				.setPeriodoRecolhimentoSemanaAnterior(this.getPeriodoSemanaAnterior(periodoRecolhimento.getDe()));
		...
		this.atualizarEncalheSedeAtendidaDosProdutos(produtosRecolhimento,
	...
	private TreeSet<Date> obterDatasRecolhimentoDisponiveis(TreeSet<Date> datasRecolhimentoFornecedor) {
		...
		for (Date dataRecolhimentoFornecedor : datasRecolhimentoFornecedor) {
			...
			isMatrizRecolhimentoConfirmado = this.lancamentoRepository
					.existeMatrizRecolhimentoConfirmado(dataRecolhimentoFornecedor);

			isAnteriorDataOperacao = DateUtil.isDataInicialMaiorDataFinal(dataOperacao, dataRecolhimentoFornecedor);
	...
	public Intervalo<Date> getPeriodoRecolhimento(Integer anoNumeroSemana) {
		...
		final int codigoInicioSemana = this.distribuidorRepository.buscarInicioSemanaRecolhimento()
				.getCodigoDiaSemana();
		...
		final Date dataInicioSemana = SemanaUtil.obterDataDaSemanaNoAno(numeroSemana, codigoInicioSemana, anoBase);
	...
	public TreeSet<Date> obterDatasRecolhimentoFornecedor(Intervalo<Date> periodoRecolhimento,
		...
		if (periodoRecolhimento == null || listaIdsFornecedores == null) {
			...
			throw new ValidacaoException(TipoMensagem.ERROR, "periodo ou fornecedores nulos");
		}

		List<DistribuicaoFornecedor> listaDistribuicaoFornecedor = this.distribuidorRepository
				.buscarDiasDistribuicaoFornecedor(listaIdsFornecedores, OperacaoDistribuidor.RECOLHIMENTO);

		if (listaDistribuicaoFornecedor == null || listaDistribuicaoFornecedor.isEmpty()) {

			throw new ValidacaoException(TipoMensagem.WARNING,
					// "Dias de recolhimento para os fornecedores não
					// encontrados!");
		...
		TreeSet<Date> datasRecolhimentoFornecedor = this.obterDatasRecolhimento(periodoRecolhimento,
				diasSemanaFornecedor);
	...
	private TreeSet<Date> obterDatasRecolhimento(Intervalo<Date> periodoRecolhimento,
		...
		TreeSet<Date> datasRecolhimento = SemanaUtil.obterPeriodoDeAcordoComDiasDaSemana(periodoRecolhimento.getDe(),
				periodoRecolhimento.getAte(), diasRecolhimentoSemana);
	...
	public void voltarConfiguracaoOriginal(Integer numeroSemana, List<Long> fornecedores, Usuario usuario) {
		...
		List<Lancamento> lancamentos = lancamentoRepository.obterLancamentosARecolherNaSemana(periodoRecolhimento,
				fornecedores);
	...
	@Transactional
	@Override
	public void processarProdutosProximaSemanaRecolhimento(List<ProdutoRecolhimentoDTO> produtos,
	...
	private void obterCotasOperacaoDiferenciada(Intervalo<Date> periodoRecolhimento,
			List<ProdutoRecolhimentoDTO> produtosRecolhimento, RecolhimentoDTO dadosRecolhimento) {

		Map<Long, List<Date>> mapCotasDiasRecolhimento = this.obterCotasOperacaoDiferenciada(periodoRecolhimento);
		...
		List<CotaOperacaoDiferenciadaDTO> cotasOperacaoDiferenciada = this.lancamentoRepository
				.obterLancamentosEncalhesPorCota(mapCotasDiasRecolhimento.keySet(), idsLancamento);
	...
	private Map<Long, List<Date>> obterCotasOperacaoDiferenciada(Intervalo<Date> periodoRecolhimento) {
		...
		List<GrupoCota> gruposCota = this.grupoRepository.obterGruposCota(periodoRecolhimento.getDe());
		...
		for (GrupoCota grupoCota : gruposCota) {
			...
			if (cotas != null && !cotas.isEmpty()) {
				...
				this.montarMapOperacaoDiferenciadaCotas(cotas, diasRecolhimento, periodoRecolhimento,
						mapCotasDiasRecolhimento);

			} else if (municipios != null && !municipios.isEmpty()) {

				this.montarMapOperacaoDiferenciadaMunicipios(municipios, diasRecolhimento, periodoRecolhimento,
						mapCotasDiasRecolhimento);
			}
		}
	...
	private void montarMapOperacaoDiferenciadaCotas(Set<Cota> cotas, Set<DiaSemana> diasRecolhimento,
			Intervalo<Date> periodoRecolhimento, Map<Long, List<Date>> mapCotasDiasRecolhimento) {

		List<Date> datasRecolhimento = this.obterDatasRecolhimentoGrupo(diasRecolhimento, periodoRecolhimento);
	...
	private void montarMapOperacaoDiferenciadaMunicipios(Set<String> municipios, Set<DiaSemana> diasRecolhimento,
			Intervalo<Date> periodoRecolhimento, Map<Long, List<Date>> mapCotasDiasRecolhimento) {
	...
	private List<Date> obterDatasRecolhimentoGrupo(Set<DiaSemana> diasRecolhimento,
		...
		List<Calendar> datasDaSemana = this.getDatasDaSemana(periodoRecolhimento);
	...
	private List<Calendar> getDatasDaSemana(Intervalo<Date> periodoRecolhimento) {
		...
		while (dataPeriodoRecolhimento.compareTo(dataInicio) >= 0 && dataPeriodoRecolhimento.compareTo(dataFim) <= 0) {
			...
			for (ProdutoRecolhimentoDTO produtoRecolhimento : produtosRecolhimento) {
				...
				if (produtoRecolhimento.isProdutoAgrupado()) {
					...
					for (Long idLancamento : produtoRecolhimento.getIdsLancamentosAgrupados()) {
						...
						List<CotaOperacaoDiferenciadaDTO> cotasOperacaoDiferenciadaDoLancamento = this
								.obterCotasOperacaoDiferenciadaPorLancamento(cotasOperacaoDiferenciada, idLancamento);

						if (!cotasOperacaoDiferenciadaDoLancamento.isEmpty()) {

							this.montarMapsOperacaoDiferenciada(cotasOperacaoDiferenciadaDoLancamento,
									mapOperacaoDifAdicionar, mapOperacaoDifRemover, dataRecolhimento);
						}
					}
				}

				List<CotaOperacaoDiferenciadaDTO> cotasOperacaoDiferenciadaDoLancamento = this
						.obterCotasOperacaoDiferenciadaPorLancamento(cotasOperacaoDiferenciada,
								produtoRecolhimento.getIdLancamento());

				if (!cotasOperacaoDiferenciadaDoLancamento.isEmpty()) {

					this.montarMapsOperacaoDiferenciada(cotasOperacaoDiferenciadaDoLancamento, mapOperacaoDifAdicionar,
	...
	private void atualizarEncalheSedeAtendidaDosProdutos(List<ProdutoRecolhimentoDTO> produtosRecolhimento,
			List<CotaOperacaoDiferenciadaDTO> cotasOperacaoDiferenciada) {
	...
	private void atualizarEncalheSedeAtendida(ProdutoRecolhimentoDTO produtoRecolhimento,
		...
		for (CotaOperacaoDiferenciadaDTO cotaOperacaoDiferenciada : cotasOperacaoDiferenciadaDoLancamento) {
			...
			expectativaEncalheAtendida = expectativaEncalheAtendida
					.add(cotaOperacaoDiferenciada.getExpectativaEncalhe());
		...
		if (produtoRecolhimento.getExpectativaEncalheAtendida() != null) {
			...
			produtoRecolhimento.setExpectativaEncalheAtendida(
					produtoRecolhimento.getExpectativaEncalheAtendida().add(expectativaEncalheAtendida));
		} else {
	...
	private List<CotaOperacaoDiferenciadaDTO> obterCotasOperacaoDiferenciadaPorLancamento(
			List<CotaOperacaoDiferenciadaDTO> cotasOperacaoDiferenciada, Long idLancamento) {
	...
	private void montarMapsOperacaoDiferenciada(List<CotaOperacaoDiferenciadaDTO> cotasOperacaoDiferenciadaDoLancamento,
			...
			Map<Date, List<CotaOperacaoDiferenciadaDTO>> mapOperacaoDifRemover, Date dataRecolhimento) {
		...
		for (CotaOperacaoDiferenciadaDTO cotaOperacaoDiferenciada : cotasOperacaoDiferenciadaDoLancamento) {
			...
			if (!datasRecolhimento.contains(dataRecolhimento)) {
				...
				Date dataRecolhimentoEscolhida = this.obterDataRecolhimentoOperacaoDiferenciada(datasRecolhimento,
						dataRecolhimento);
	...
	private void addMap(Map<Date, List<CotaOperacaoDiferenciadaDTO>> map, Date dataRecolhimento,
	 ...
	 */
	private Date obterDataRecolhimentoOperacaoDiferenciada(List<Date> datasRecolhimento, Date dataRecolhimento) {
	...
	private List<ProdutoRecolhimentoDTO> tratarAgrupamentoLancamentosComMesmaEdicao(
		...
		Map<Long, List<Long>> mapProdutoEdicaoLancamento = this.montarMapProdutoEdicaoLancamento(produtosRecolhimento);
		...
		for (ProdutoRecolhimentoDTO produtoRecolhimento : produtosRecolhimento) {
			...
			if (this.existeMaisDeUmLancamento(idLancamentos)) {
				...
				this.agruparProdutosRecolhimento(produtoRecolhimento, idLancamentos, produtosRecolhimento,
						produtosRecolhimentoAgrupados);
			}
	...
	private void agruparProdutosRecolhimento(ProdutoRecolhimentoDTO produtoRecolhimento, List<Long> idLancamentos,
			List<ProdutoRecolhimentoDTO> produtosRecolhimento,
		...
		for (Long idLancamento : idLancamentos) {
			...
			ProdutoRecolhimentoDTO produtoRecolhimentoMesmaEdicao = this.obterProdutoRecolhimento(produtosRecolhimento,
					idLancamento);
			...
			this.efetivarAgrupamento(produtoRecolhimento, produtoRecolhimentoAntigo, produtoRecolhimentoNovo,
					idLancamento);
	...
	private void efetivarAgrupamento(ProdutoRecolhimentoDTO produtoRecolhimento,
			ProdutoRecolhimentoDTO produtoRecolhimentoAntigo, ProdutoRecolhimentoDTO produtoRecolhimentoNovo,
			Long idLancamento) {

		BigDecimal encalhe = BigDecimalUtil.soma(produtoRecolhimentoAntigo.getExpectativaEncalhe(),
				produtoRecolhimentoNovo.getExpectativaEncalhe());

		BigDecimal encalheAtendida = BigDecimalUtil.soma(produtoRecolhimentoAntigo.getExpectativaEncalheAtendida(),
				produtoRecolhimentoNovo.getExpectativaEncalheAtendida());

		BigDecimal encalheSede = BigDecimalUtil.soma(produtoRecolhimentoAntigo.getExpectativaEncalheSede(),
				produtoRecolhimentoNovo.getExpectativaEncalheSede());

		BigDecimal valorTotal = BigDecimalUtil.soma(produtoRecolhimentoAntigo.getValorTotal(),
		...
		produtoRecolhimento
				.setDataRecolhimentoDistribuidor(produtoRecolhimentoAntigo.getDataRecolhimentoDistribuidor());
	...
	private boolean isLancamentoMaisAntigo(ProdutoRecolhimentoDTO produtoRecolhimentoDTO,
		...
		if (produtoRecolhimentoDTO.getDataLancamento().before(produtoRecolhimento.getDataLancamento())) {
	...
	private void removerChamadaEncalheCotaEChamadaEncalhe(List<Long> listaIdChamadaEncalheRemover) {
		...
		if (!listaIdChamadaEncalheRemover.isEmpty() && listaIdChamadaEncalheRemover.size() > 0) {
			...
			this.chamadaEncalheCotaRepository
					.removerChamadaEncalheCotaPorIdsChamadaEncalhe(listaIdChamadaEncalheRemover);
	...
	public String reabrirMatriz(List<Date> datasConfirmadas, Usuario usuario) {
		...
		this.validarReaberturaMatriz(datasConfirmadas, this.distribuidorService.obterDataOperacaoDistribuidor());
	...
	public String cadeadoMatriz(List<Date> datasConfirmadas, Usuario usuario) {
		...
		List<Lancamento> lancamentos = this.lancamentoRepository
				.obterRecolhimentosEmBalanceamentoRecolhimento(datasConfirmadas);
	...
	private void validarReaberturaMatriz(List<Date> datasConfirmadas, Date dataOperacao) {
		...
		for (Date dataConfirmada : datasConfirmadas) {
			...
			if (dataConfirmada.compareTo(dataOperacao) <= 0) {
				...
				mensagens.add(
						"Para reabrir a matriz, a data (" + dataFormatada + ") deve ser maior que a data de operação!");
			}
		}
	...
	public Date obterDataRecolhimentoValido(Date dataLancamento, Long idFornecedor) {
		...
		Long idDinap = fornecedorService
				.obterFornecedorPorCodigoInterface(new Integer(distribuidor.getCodigoDistribuidorDinap())).getId();
		Long idFc;

		if (distribuidor.getCodigoDistribuidorFC() != null
				&& !distribuidor.getCodigoDistribuidorFC().trim().equals("")) {

			Fornecedor f = fornecedorService
					.obterFornecedorPorCodigoInterface(Integer.valueOf(distribuidor.getCodigoDistribuidorFC()));
			if (f != null) {
