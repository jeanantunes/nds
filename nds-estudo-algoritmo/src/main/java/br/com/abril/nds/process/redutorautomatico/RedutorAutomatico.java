 ...
 * - N/A
 * 
 * Processo Anterior: {@link AjusteReparte} Próximo Processo: {@link ReparteMinimo}</p>
 */
public class RedutorAutomatico extends ProcessoAbstrato {
	...
	BigDecimal excedente = estudo.getReparteDistribuir().subtract(
		estudo.getSomatoriaVendaMedia());
	BigDecimal percentualExcedente = excedente.divide(
		estudo.getSomatoriaVendaMedia(), 2, BigDecimal.ROUND_FLOOR);
	BigDecimal menorVenda = new BigDecimal(0);

	if ((percentualExcedente.doubleValue() > 0.4d)
		&& (percentualExcedente.doubleValue() < 0.6d)) {
	...
	for (Cota cota : estudo.getCotas()) {
	    if ((cota.getVendaMedia().doubleValue() <= menorVenda.doubleValue())
		    && (cota.getVendaEdicaoMaisRecenteFechada()
