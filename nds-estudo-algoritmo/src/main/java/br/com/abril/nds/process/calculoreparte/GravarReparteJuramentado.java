...
public class GravarReparteJuramentado extends ProcessoAbstrato {
    	...
    	if(getEstudo().getProduto().isParcial() ){
    		for(Cota cota:getEstudo().getCotas()){
    			...
    			if(qtdeVezesEnviada>=2){
    				...
    				BigDecimal reparteJuramentadoAFaturar = movimentoEstoqueCotaDAO.retornarReparteJuramentadoAFaturar(cota, getEstudo().getProduto());
	...
	private void fimProcesso() {
		...
		// Fim do sub processo
		// Se houver saldo no reparte total distribuÌdo, n„o considerando-se o
		// total de reparte juramentado:
		// Indice de Sobra ou Falta = ( 'sum'ReparteCalculado Cota /
		// ReparteCalculado) * ReparteCalculado Cota (n„o
		...
		for (Cota cota : getEstudo().getCotas()) {
			...
			if (!cota.getClassificacao()
				...
				// (exceto repartes fixados (FX), quantidades M¡XIMAS E MÕNIMAS
				// (MM)
				// e bancas com MIX (MX)).
