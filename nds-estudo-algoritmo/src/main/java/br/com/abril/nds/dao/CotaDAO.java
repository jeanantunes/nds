...
public class CotaDAO {
	...
	try {
	    PreparedStatement psmt = Conexao
		    .getConexao()
		    .prepareStatement(
			    "SELECT C.*, P.NOME FROM COTA C INNER JOIN PESSOA P ON (P.ID = C.PESSOA_ID) ORDER BY C.ID");
	...
	try {
	    PreparedStatement psmt = Conexao
		    .getConexao()
		    .prepareStatement(
			    "SELECT AJUSTE_APLICADO, FORMA_AJUSTE FROM AJUSTE_REPARTE WHERE COTA_ID = ?");
		...
		if (rs.getString("FORMA_AJUSTE").equals("vendaMedia")) {
		    ...
		    cota.setPercentualEncalheMaximo(rs
			    .getBigDecimal("AJUSTE_APLICADO"));
		}
	    ...
	    System.out
		    .println("Ocorreu um erro ao consultar os parâmetros das cotas");
	    ex.printStackTrace();
	}
	return cota;
    }
