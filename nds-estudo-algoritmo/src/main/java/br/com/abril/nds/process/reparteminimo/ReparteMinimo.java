...
/**
 * Processo que faz o ajuste do Reparte Mínimo para as cotas de acordo com os
 * parâmetros do setup, com o pacote padrão e com a quantidade de reparte que
 * ele deverá receber, para evitar que ele receba uma quantidade menor que o
 * mínimo configurado ou que o pacote padrão do produto.
 * <p style="white-space: pre-wrap;">
 * SubProcessos: - N/A Processo Pai: - N/A
 * 
 * Processo Anterior: {@link RedutorAutomatico} Próximo Processo:
	...
	public void executar(EstudoTransient estudo) throws Exception {
		if (estudo.isDistribuicaoPorMultiplos() && (estudo.getPacotePadrao() != null)) {
			...
			for (CotaEstudo cota : estudo.getCotas()) {
				...
				BigDecimal pacPadrao = new BigDecimal(estudo.getPacotePadrao());
				if (estudo.getPacotePadrao().compareTo(BigInteger.ZERO) > 0) {
					reparteMinimo = reparteMinimo.divide(pacPadrao, 0, BigDecimal.ROUND_FLOOR).multiply(pacPadrao);
				}
			...
			if (estudo.getReparteDistribuir().compareTo(BigInteger.ZERO) > 0) {
				if (somatoriaReparteMinimo.divide(new BigDecimal(estudo.getReparteDistribuir()), 2, BigDecimal.ROUND_HALF_UP).compareTo(BigDecimal.valueOf(0.75)) > 0) {
					throw new Exception(
