...
public class ReparteMinimo extends ProcessoAbstrato {
	...
	if (estudo.getParametro().isDistribuicaoPorMultiplos()) {
		...
		cota.setReparteMinimo(cota
			.getReparteMinimo()
			.divide(estudo.getParametro().getPacotePadrao(), 0,
				BigDecimal.ROUND_FLOOR)
			.multiply(estudo.getParametro().getPacotePadrao()));
		if (cota.getReparteMinimo().equals(BigDecimal.ZERO)) {
		    cota.setReparteMinimo(estudo.getParametro()
			    .getPacotePadrao());
		}
		somatoriaReparteMinimo.add(cota.getReparteMinimo());
	    }
	    if (somatoriaReparteMinimo.divide(estudo.getReparteDistribuir(), 0,
		    BigDecimal.ROUND_FLOOR).doubleValue() > new BigDecimal(
		    0.75f).doubleValue()) {
		throw new Exception("");
	    }
	}
