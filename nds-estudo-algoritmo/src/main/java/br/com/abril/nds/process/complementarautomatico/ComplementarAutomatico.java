...
public class ComplementarAutomatico extends ProcessoAbstrato {
	...
	if ((estudo.getProdutoEdicaoEstudo() != null) && (estudo.getEdicoesBase() != null)) {
		...
		if (estudo.getPercentualExcedente().compareTo(BigDecimal.ONE) > 0) {
			...
			if (BigDecimal.valueOf(200).divide(estudo.getExcedente(), 2, BigDecimal.ROUND_FLOOR).compareTo(BigDecimal.valueOf(10)) < 0) {
			    if (BigDecimal.valueOf(2).compareTo(estudo.getExcedente().multiply(BigDecimal.valueOf(0.02))) > 0) {
