 ...
 * ele atribui ao ReparteCalculado da cota a soma da VendaMediaFinal ao valor
 * informado na tela Ajuste de Reparte (se ele for menor que o Pacote Padrão
 * definido, será somado o Pacote Padrão ao invés desse valor).
 * <p style="white-space: pre-wrap;">
 * SubProcessos: - N/A Processo Pai: - N/A
 * 
 * Processo Anterior: {@link VendaMediaFinal} Próximo Processo:
...
public class AjusteReparte extends ProcessoAbstrato {
	...
	for (Cota cota : estudo.getParametro().getCotas()) {
		...
		if (cota.getAjusteReparte().intValue() > estudo.getParametro()
			.getPacotePadrao().intValue()) {
		    ajusteReparte = cota.getAjusteReparte();
		} else {
		    ajusteReparte = estudo.getParametro().getPacotePadrao();
		}
		cota.setReparteCalculado(cota.getVendaMediaFinal().add(
			ajusteReparte));
	    }
	}
    }
