...
public class RecebimentoFisicoRepositoryImplTest extends
		AbstractRepositoryImplTest {

	@Autowired
	private RecebimentoFisicoRepository recebimentoFisicoRepositoryImpl;
	...
	public void setup() {
		...
		notaFiscal = Fixture.notaFiscalEntradaFornecedor(cfop, fornecedor,
				tipoNotaFiscal, usuario, new BigDecimal(10),
		...
		tipoProduto = Fixture.tipoProduto("Revista", GrupoProduto.REVISTA,
				ncmRevistas, null, 13L);
		save(tipoProduto);

		Editor abril = Fixture.editoraAbril();
		save(abril);

		produto = Fixture.produto("1", "Revista Veja", "Veja",
				PeriodicidadeProduto.SEMANAL, tipoProduto, 5, 5,
				new Long(10000), TributacaoFiscal.TRIBUTADO);
		produto.addFornecedor(dinap);
		produto.setEditor(abril);
		save(produto);

		produtoEdicao = Fixture.produtoEdicao(1L, 10, 14, new Long(100),
				BigDecimal.TEN, new BigDecimal(20), "ABCDEFGHIJKLMNOPQ",
				produto, null, false);
		save(produtoEdicao);

		itemNotaFiscal = Fixture.itemNotaFiscal(produtoEdicao, usuario,
				notaFiscal, new Date(), new Date(), TipoLancamento.LANCAMENTO,
				BigInteger.valueOf(12));
		save(itemNotaFiscal);

		recebimentoFisico = Fixture.recebimentoFisico(notaFiscal, usuario,
				new Date(), new Date(), StatusConfirmacao.PENDENTE);
		save(recebimentoFisico);

		itemRecebimentoFisico = Fixture.itemRecebimentoFisico(itemNotaFiscal,
				recebimentoFisico, BigInteger.valueOf(12));
	...
	public void inserirDadosRecebimentoFisico() {
		...
		recebimentoDTO
				.setDataRecolhimento(new Date(System.currentTimeMillis()));
		...
		NotaFiscalEntrada notaFiscalFromBD = notaFiscalRepository
				.buscarPorId(notaFiscal.getId());

		recebimentoFisicoService.inserirDadosRecebimentoFisico(usuario,
				notaFiscalFromBD, listaDTO, new Date());
	...
	public void obterListaItemRecebimentoFisico() {
		...
		List<RecebimentoFisicoDTO> lista = recebimentoFisicoRepositoryImpl
				.obterListaItemRecebimentoFisico(idNotaFiscal);
	...
	@Test
	public void obterRecebimentoFisicoPorNotaFiscal() {
		Long idNotaFiscal = 1L;
		RecebimentoFisico recebimentoFisico = recebimentoFisicoRepositoryImpl
				.obterRecebimentoFisicoPorNotaFiscal(idNotaFiscal);
	}

	@Test
	public void obterItensRecebimentoFisicoDoProduto() {
		Long idProdutoEdicao = 1L;
		List<ItemRecebimentoFisico> lista = recebimentoFisicoRepositoryImpl
				.obterItensRecebimentoFisicoDoProduto(idProdutoEdicao);
