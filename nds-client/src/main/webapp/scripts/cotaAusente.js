var cotaAusenteController = $.extend(true, {
    ...
    abrirModalCotaAusente: function (id) {

        $("#flexiGridRateios", cotaAusenteController.workspace).flexigrid($.extend({}, {
            dataType: 'json',
            colModel: [
                {
                    display: 'Nº Cota',
        ...
        $("#dialog-rateios", cotaAusenteController.workspace).dialog({
            ...
            form: $("#dialog-rateios", cotaAusenteController.workspace).parents("form")
        });


        $.getJSON(contextPath + "/cotaAusente/getCotaRateioDeCotaAusente",
            {"cotaAusenteId": id},
            function (result) {
                ...
                $("#flexiGridRateios").flexAddData(data).flexReload();
    ...
    processaRetornoPesquisa: function (result) {
        ...
        $.each(grid.rows, function (index, row) {
            ...
            if (row.cell.destino && row.cell.destino.toLocaleLowerCase() != 'suplementar') {
                row.cell.destino = "<a href='javascript:cotaAusenteController.abrirModalCotaAusente(" + (row.cell.idCotaAusente) + ");' >" + row.cell.destino + "</a>"; //cotaAusenteController.gerarBotaoExcluir(row.cell.idCotaAusente, row.cell.cotaAvista);
            }
            ;
            ...
            buttons: {
                "Confirmar": function () {
                    ...
                    $('.cotaOrigem').each(function () {
                        if (this.value.length > 0) cotas.push(this.value);
                    });
    ...
    gerarNovoRateio: function (indiceLinhaAlterada) {
        ...
        if (indiceLinhaAlterada == (qtdeRateios)) {
            ...
            cotaAusenteController.movAtual.rateios[indiceLinhaAlterada] = {
                "numCota": numCota,
                "nomeCota": nomeCota,
                "qtde": qtde
            };
        ...
        $.each(cotaAusenteController.mov, function (index, movimento) {
            ...
            if (movimento.rateios) {
                ...
                $.each(movimento.rateios, function (indexR, rateio) {
                    parametros.push({
                        name: 'movimentos[' + index + '].rateios[' + indexR + '].numCota',
                        value: rateio.numCota
                    });
                    parametros.push({
                        name: 'movimentos[' + index + '].rateios[' + indexR + '].nomeCota',
                        value: rateio.nomeCota
                    });
                    parametros.push({
                        name: 'movimentos[' + index + '].rateios[' + indexR + '].qtde',
                        value: rateio.qtde
                    });
                });
            }
