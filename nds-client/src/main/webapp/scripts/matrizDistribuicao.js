function MatrizDistribuicao(pathTela, descInstancia, workspace) {
	...
	this.isCliquePesquisar;
			...
			this.pesquisar = function() {
				...
				data.push({
					name : 'dataLancamento',
					value : $("#datepickerDe", _workspace).val()
				});

				$.postJSON(pathTela
						+ "/matrizDistribuicao/obterMatrizDistribuicao", data,
						function(result) {
			...
			this.carregarGrid = function() {
				...
				$(".lancamentosProgramadosGrid", _workspace)
						.flexOptions(
								{
									url : pathTela
											+ "/matrizDistribuicao/obterGridMatrizDistribuicao",
									dataType : 'json',
									autoload : false,
									singleSelect : true,
			...
			this.confirmarPaginacao = function(elemento) {
				...
				var noSelect = $('[name=checkgroup]:checked', _workspace)
						.size() == 0;
				...
				$("#dialog-pagincao-confirmada", _workspace)
						.dialog(
								{
									resizable : false,
									height : 'auto',
									...
									form : $("#dialog-pagincao-confirmada",
											this.workspace).parents("form")
								});
			...
			this.onSuccessPesquisa = function() {
				...
				$(T.linhasDestacadas).each(
						function(i, item) {
							id = '#row' + item;
							$(id, _workspace).removeClass("erow").addClass(
									"gridLinhaDestacada");
							$(id, _workspace).children("td").removeClass(
									"sorted");
						});
			...
			this.processaRetornoPesquisa = function(resultadoPesquisa) {
				...
				$.each(resultadoPesquisa[0].rows, function(index, row) {
					T.processarLinha(index, row);
				});
			...
			this.processarLinha = function(i, row) {
				...
				if (liberado) {
					imgLiberado = '<img title="Liberado" src="'
							+ contextPath
							+ '/images/ico_check.gif" hspace="5" border="0px" />';
				} else {
					imgLiberado = '';
				}

				row.cell.liberado = imgLiberado;
				row.cell.reprogramar = T.gerarCheckDistribuicao(
						row.cell.idLancamento, i);

				T.formataCampos(row);

				var repDist = (row.cell.lancto - row.cell.promo);

				row.cell.sobra = '<span id="sobra' + i + '">' + repDist
						+ '</span>';
			...
			this.gerarInputRepDistrib = function(valor, index) {
				...
				return '<input id="inputRepDistrib'
						+ index
						+ '" onchange="'
						+ T.instancia
						+ '.alterarReparte(this,\''
						+ index
						+ '\');" type="text" name="repDistrib" style="width:60px; float:left;" value="'
						+ valor + '" />';
			...
			this.alterarReparte = function(input, index) {
				...
				var vlr = (T.lancamentos[index].lancto
						- T.lancamentos[index].promo - T.lancamentos[index].repDistrib);
				$("#sobra" + index, _workspace).text(vlr);
				T.lancamentos[index].sobra = vlr;
			},

			this.gerarCheckDistribuicao = function(id, index) {
				return '<input id="checkDistribuicao' + index
						+ '" type="checkbox" value="' + id
						+ '" name="checkgroup" ' + ' onclick="' + T.instancia
						+ '.selecionarCheck(this,\'' + index + '\');" />';
			},
			...
			this.confirmarFinalizacaoDeMatriz = function() {
				...
				$.each(T.lancamentos, function(index, lancamento) {
					data.push({
						name : 'produtosDistribuicao[' + index + '].idEstudo',
						value : lancamento.estudo
					});
					data
							.push({
								name : 'produtosDistribuicao[' + index
										+ '].repDistrib',
								value : lancamento.repDistrib
							});
				});

				$.postJSON(pathTela
						+ "/matrizDistribuicao/finalizarMatrizDistribuicao",
			...
			this.confirmarReaberturaDeMatriz = function() {
				...
				$.postJSON(pathTela
						+ "/matrizDistribuicao/reabrirMatrizDistribuicao",
						null, function() {
							T.checkUncheckLancamentos(false);
							T.carregarGrid();
							T.exibirMensagemSucesso();
				...
				$.each(T.lancamentos, function(index, lancamento) {
					if (lancamento.selecionado) {
						...
						data.push({
							name : 'produtosDistribuicao[' + index
									+ '].numeroEdicao',
							value : lancamento.edicao
						});
						data.push({
							name : 'produtosDistribuicao[' + index
									+ '].codigoProduto',
							value : lancamento.codigoProduto
						});
					}
				});

				$.postJSON(pathTela
						+ "/matrizDistribuicao/excluirEstudosSelecionados",
						data, function() {
							T.checkUncheckLancamentos(false);
							T.atualizarGrid();
							T.exibirMensagemSucesso();
			...
			this.confirmarReabrirEstudo = function() {
				...
				$.postJSON(pathTela
						+ "/matrizDistribuicao/reabrirEstudosSelecionados",
						data, function() {
							T.checkUncheckLancamentos(false);
							T.atualizarGrid();
							T.exibirMensagemSucesso();
			...
			this.popup_confirmar_duplicarLinha = function() {
				...
				$("#dialog-confirm-duplicar", _workspace)
						.dialog(
								{
									resizable : false,
									height : 'auto',
				...
				$("#dialog-confirm-finalizacao", _workspace).dialog(
						{
							...
							form : $("#dialog-confirm-finalizacao",
									this.workspace).parents("form"),
							beforeClose : function() {
								clearMessageDialogTimeout("dialog-confirmar");
							}
				...
				$("#dialog-confirm-reabrir-matriz", _workspace).dialog(
						{
							...
							form : $("#dialog-confirm-reabrir-matriz",
									this.workspace).parents("form"),
							beforeClose : function() {
								clearMessageDialogTimeout("dialog-confirmar");
							}
			...
			this.popup_confirmar_exclusao_estudo = function() {
				...
				$("#dialog-confirm-exclusao", _workspace)
						.dialog(
								{
									resizable : false,
									height : 'auto',
				...
				$("#dialog-confirm-reabert", _workspace).dialog(
						{
							...
							form : $("#dialog-confirm-reabert", this.workspace)
									.parents("form"),
							beforeClose : function() {
								clearMessageDialogTimeout("dialog-confirmar");
							}
			...
			this.atualizarResumoBalanceamento = function() {
				...
				$.postJSON(pathTela
						+ "/matrizDistribuicao/atualizarResumoBalanceamento",
			...
			this.validarMarcacaoUnicoItem = function() {
				...
				if (T.obeterQuantosItensMarcados() > 1) {
					exibirMensagem("WARNING",
							[ "Apenas um item pode ser marcado." ]);
					return false;
				}
			...
			this.confirmaDuplicaoLinha = function() {
				...
				data.push({
					name : 'produtoDistribuicao.idLancamento',
					value : selecionado.idLancamento
				});
				data.push({
					name : 'produtoDistribuicao.numeroEdicao',
					value : selecionado.edicao
				});
				data.push({
					name : 'produtoDistribuicao.codigoProduto',
					value : selecionado.codigoProduto
				});

				$.postJSON(pathTela + "/matrizDistribuicao/duplicarLinha",
						data, function(result) {
							T.checkUncheckLancamentos(false);
							T.carregarGrid();
							T.exibirMensagemSucesso();
			...
			this.pesquisarProdutos = function() {
				$('#workspace').tabs('addTab', "Informações do Produto",
						contextPath + "/distribuicao/informacoesProduto/");
			...
			this.somarEstudos = function() {
				...
				if (selecionado.estudo == null || selecionado.estudo == "") {
					exibirMensagem(
							"WARNING",
							[ "Selecione um produto que tenha um estudo gerado." ]);
					return;
				}

				if (selecionado.liberado) {
					exibirMensagem("WARNING",
							[ "Não é permitido somar estudos liberados." ]);
					return;
				}
			...
			this.copiarProporcionalDeEstudo = function() {
				...
				if (selecionado.estudo != null && selecionado.estudo != "") {
					exibirMensagem(
							"WARNING",
							[ "Selecione um produto que não tenha um estudo gerado." ]);
					return;
				}
			...
			this.carregarProdutoPorEstudo = function() {
				...
				$.postJSON(pathTela
						+ "/matrizDistribuicao/carregarProdutoEdicaoPorEstudo",
						data, function(result) {
			...
			this.confirmarCopiarProporcionalDeEstudo = function() {
				...
				if ($('#copiarEstudo-estudo').text() != '') {
					exibirMensagem("WARNING",
							[ "O estudo já foi gerado para este produto." ]);
					return;
				}
				...
				if (estudo == null || estudo == "") {
					exibirMensagem("WARNING",
							[ "Pesquise um estudo para a copia!" ]);
					return;
				}

				var fixacao = $('#copiarEstudo-fixacao').attr("checked") == 'checked';
				var pctPadrao = ($('#copiarEstudo-multiplos-check').attr(
						"checked") == 'checked') ? $('#copiarEstudo-pctPadrao')
						.val() : null;
				var reparteDistribuido = $("#copiarEstudo-reparteDistribuido")
						.text();
				...
				data.push({
					name : 'copiaProporcionalDeDistribuicaoVO.idLancamento',
					value : idLancamento
				});
				data.push({
					name : 'copiaProporcionalDeDistribuicaoVO.idEstudo',
					value : estudo
				});
				data.push({
					name : 'copiaProporcionalDeDistribuicaoVO.fixacao',
					value : fixacao
				});
				data
						.push({
							name : 'copiaProporcionalDeDistribuicaoVO.reparteDistribuido',
							value : reparteDistribuido
						});

				if (pctPadrao != null) {
					data
							.push({
								name : 'copiaProporcionalDeDistribuicaoVO.pacotePadrao',
								value : pctPadrao
							});
				}
				...
				$
						.postJSON(
								pathTela
										+ "/matrizDistribuicao/confirmarCopiarProporcionalDeEstudo",
								data,
								function(result) {
									T.exibirMensagemSucesso();
									$("#copiarEstudo-estudo").text(result.long);
									$("#copiarEstudo-estudoPesquisa").attr(
											'disabled', 'true');
									$("#copiarEstudo-reparteDistribuido").text(
											"");
									T.atualizarGrid();
			...
			this.confirmarSomaDeEstudos = function() {
				...
				if (estudoPesquisa == null || estudoPesquisa == "") {
					exibirMensagem("WARNING",
							[ "Pesquise um estudo para a soma!" ]);
					return;
				}
				...
				data.push({
					name : 'idEstudoBase',
					value : estudoPesquisa
				});
				data.push({
					name : 'distribuicaoVO.idEstudo',
					value : estudo
				});
				data.push({
					name : 'distribuicaoVO.codigoProduto',
					value : codigoProduto
				});
			...
			this.mostrarOpcoes = function() {
				...
				$('.setaMuda').attr(
						'src',
						(opcoesAberto) ? contextPath
								+ '/images/p7PM_dark_south_1.gif' : contextPath
								+ '/images/p7PM_dark_south.gif');
			},
			...
			this.esconderOpcoes = function() {
				setTimeout(function() {
					$('.opcoesEstudos').hide();
					$('.setaMuda').attr('src',
							contextPath + '/images/p7PM_dark_south.gif');
				}, 2000);
			};
	...
	this.analise = function() {
		...
		if (T.validarMarcacaoUnicoItem()) {
			...
			if (selecionado.estudo == null || selecionado.estudo == "") {
				...
				$.get(pathTela +'/matrizDistribuicao/histogramaPosEstudo', // url
					null, // parametros
					function(html) { // onSucessCallBack
						...
						for ( var prop in selecionado) {
							params.push({
								name : "selecionado." + prop,
								value : selecionado[prop]
							});
						}
	...
	this.gerarEstudoManual = function() {
		...
		params += '&produto.idEstudo='+ selecionado.estudo;
