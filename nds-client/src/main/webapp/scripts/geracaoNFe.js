...
GeracaoNFe.prototype.init = function() {
	...
	$('#checkboxCheckAllCotasSuspensas').change(function() {
		$(".checkboxCheckCotasSuspensas").attr('checked', $(this).attr('checked') == 'checked');
	});
...
GeracaoNFe.prototype.gerar = function() {
	...
	if(todas){
		var listCotasSuspensas =  $.map( this.mapCotasSuspensas, function( value, index ) {		
			if(value){
				return index;
			}
	    }); 	
		params = serializeArrayToPost('idCotasSuspensas',listCotasSuspensas,params);
	}
	params['dataEmissao'] =  $("#datepickerEmissao").val();
	params['todasCotasSuspensa'] = todas;
	
	$.postJSON(this.path + 'gerar.json', params,
			function(data) {
...
GeracaoNFe.prototype.getParams =function(){
	var params =  {"intervaloBox.de": $("#inputIntervaloBoxDe").val(),
		...
		"intervaloDateMovimento.de" : $("#datepickerIntervaloMovimentoDe").val(),
		"intervaloDateMovimento.ate": $("#datepickerIntervaloMovimentoAte").val(),
		"tipoNotaFiscal": 1
	};
...
GeracaoNFe.prototype.btnGerarOnClick = function() {	
	...
	var _this = this;
	var params =  this.getParams();
	$.postJSON(this.path + 'hasCotasSuspensas.json', params,
			function(data) {
				var tipoMensagem = data.tipoMensagem;
				var listaMensagens = data.listaMensagens;
				...
				if(data.cotasSuspensas){
					...
					_this.gridReaload(_this.$gridCotasSuspensas, 'buscaCotasSuspensas.json');						
				}
...
GeracaoNFe.prototype.gridReaload = function(grid, uri) {
	...
	if(listaFornecedores){		
		$.each(listaFornecedores, function(index, value) { 
			params.push({'name':"listIdFornecedor[]",'value':value});
		});
	}
	...
	this.$gridCotasSuspensas = $("#gridCotasSuspensas").flexigrid(
			{
				...
				colModel : [ {
					...
					sortable : false,
					align : 'center',
					process: function(tdDiv, pid) {_this.processCotasSuspensas(tdDiv, pid);}
				} ],
				sortname : "codigo",
				sortorder : "asc",
				usepager : true,
