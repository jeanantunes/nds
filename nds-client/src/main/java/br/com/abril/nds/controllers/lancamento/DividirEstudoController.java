...
public class DividirEstudoController extends BaseController {
	...
	if (dataLancamentoSegundoEstudo != null && !dataLancamentoSegundoEstudo.equalsIgnoreCase("")) {
		...
		Estudo primeiroEstudo = estudoOriginal.clone();
		...
		Estudo segundoEstudo = estudoOriginal.clone();
		...
		List<Long> listIdEstudoAdiconado = this.estudoService.salvar(listEstudo);
		
		mensagensValidacao.add("Estudo dividido com sucesso!");
		...
		this.result.use(Results.json()).from(new ValidacaoVO(TipoMensagem.SUCCESS, mensagensValidacao), Constantes.PARAM_MSGS).recursive().serialize();
