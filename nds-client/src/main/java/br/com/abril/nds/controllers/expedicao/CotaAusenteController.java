...
public class CotaAusenteController extends BaseController {
	...
	@Get
	public void getCotaRateioDeCotaAusente(Long cotaAusenteId){
		List<RateioCotaVO> rateioCotaVOS = this.cotaAusenteService.obterCotaRateioDeCotaAusente(cotaAusenteId);

		this.result.use(Results.json()).withoutRoot().from(rateioCotaVOS).serialize();
