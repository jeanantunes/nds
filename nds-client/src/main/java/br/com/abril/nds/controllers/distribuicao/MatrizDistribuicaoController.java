...
public class MatrizDistribuicaoController extends BaseController {
	...
	@Post
	public void obterMatrizDistribuicao(Date dataLancamento,
	...
	public void exportar(FileType fileType) throws IOException {
		...
		TotalizadorProdutoDistribuicaoVO totalizadorProdutoDistribuicaoVO = matrizDistribuicaoService
		...
		if (totalizadorProdutoDistribuicaoVO != null && totalizadorProdutoDistribuicaoVO.getListProdutoDistribuicao() != null && 
			...
			RodapeDTO rodapeDTO = new RodapeDTO(
					CurrencyUtil.formatarValor(totalizadorProdutoDistribuicaoVO
							.getTotalEstudosGerados()),
					CurrencyUtil.formatarValor(totalizadorProdutoDistribuicaoVO
							.getTotalEstudosLiberados()));

			FileExporter.to("matriz_distribuicao", fileType).inHTTPResponse(
					this.getNDSFileHeader(),
					filtro,
					rodapeDTO,
					totalizadorProdutoDistribuicaoVO
	 ...
	 */
	private FiltroDistribuicaoDTO configurarFiltropesquisa(Date dataPesquisa,
			List<Long> listaIdsFornecedores) {

		FiltroDistribuicaoDTO filtro = new FiltroDistribuicaoDTO(dataPesquisa,
				listaIdsFornecedores);

		if (listaIdsFornecedores != null && !listaIdsFornecedores.isEmpty()) {

			filtro.setNomesFornecedor(this
					.montarNomeFornecedores(listaIdsFornecedores));
		}
	...
	private String montarNomeFornecedores(List<Long> idsFornecedores) {
		...
		List<Fornecedor> listaFornecedor = fornecedorService
				.obterFornecedoresPorId(idsFornecedores);
	...
	private void validarDadosPesquisa(Date dataPesquisa) {
		...
		if (dataPesquisa == null) {
			listaMensagens
					.add("O preenchimento do campo [Data] é obrigatório!");
		}
	...
	private FiltroDistribuicaoDTO obterFiltroSessao() {
		...
		FiltroDistribuicaoDTO filtro = (FiltroDistribuicaoDTO) this.session
	...
	public void gerarEstudoAutomatico(String codigoProduto, Long numeroEdicao,
		...
		try {
			...
			estudoAutomatico = estudoAlgoritmoService.gerarEstudoAutomatico(
					produto, reparte.toBigInteger(), this.getUsuarioLogado());
		} catch (Exception e) {
		...
		result.use(Results.json()).from(htmlEstudo, "estudo").recursive()
				.serialize();
	}
	...
	@Post
	public void somarEstudos(Long idEstudoBase,
	...
	public void histogramaPosEstudo() {
		result.forwardTo(HistogramaPosEstudoController.class)
				.histogramaPosEstudo();
	}
}
