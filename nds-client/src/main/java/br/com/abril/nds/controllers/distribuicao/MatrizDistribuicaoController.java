	...
	private String estudoToHTML(EstudoTransient estudo) {
		...
		for (CotaEstudo ce : estudo.getCotas()) {
			if (ce.getReparteCalculado().compareTo(BigInteger.ZERO) > 0) {
				...
				h.append(String.format(" <td>%s</td>", ce.getEdicoesRecebidas()
						.size()));
				h.append(String.format(" <td>%s</td>", venda));
				for (int i = 0; i < 6; i++) {
					if (i < ce.getEdicoesRecebidas().size()) {
						h.append(String.format(" <td>%s</td>", ce
								.getEdicoesRecebidas().get(i).getReparte()));
						h.append(String.format(" <td>%s</td>", ce
								.getEdicoesRecebidas().get(i).getVenda()));
						h.append(String.format(" <td>%s</td>", ce
								.getEdicoesRecebidas().get(i).getPeso()));
					} else {
						h.append(" <td></td>");
						h.append(" <td></td>");
						h.append(" <td></td>");
				...
				for (int i = 0; i < 6; i++) {
					if (i < ce.getEdicoesRecebidas().size()) {
						h.append(String.format(" <td>%s</td>", ce
								.getEdicoesRecebidas().get(i)
								.getVendaCorrigida()));
						if ((menorVenda.getVenda().compareTo(BigDecimal.ZERO) == 0)
								|| (ce.getEdicoesRecebidas().get(i).getVenda()
										.compareTo(menorVenda.getVenda()) < 0)) {
				...
				h.append(String.format(" <td>%s</td>",
						ce.getVendaMediaNominal()));
				...
				for (int i = 0; i < 6; i++) {
					if (i < ce.getEdicoesRecebidas().size()) {
						h.append(String.format(" <td>%s</td>", ce
								.getEdicoesRecebidas().get(i)
								.getDivisaoVendaCrescente()));
					} else {
						h.append(" <td></td>");
					}
				}
				h.append(String.format(" <td>%s</td>",
						ce.getIndiceCorrecaoTendencia()));
				if (ce.getVendaMediaMaisN() != null) {
					h.append(String.format(" <td>%s</td>",
							ce.getVendaMediaMaisN()));
				} else if (ce.getPercentualEncalheMaximo() != null) {
					h.append(String.format(" <td>%s</td>",
							ce.getPercentualEncalheMaximo()));
				} else {
					h.append(String.format(" <td>%s</td>",
							ce.getIndiceAjusteCota()));
				}
				h.append(String.format(" <td>%s</td>",
						ce.getIndiceTratamentoRegional()));
				h.append(String.format(" <td>%s</td>",
						ce.getIndiceVendaCrescente()));
				h.append(String.format(" <td>%s</td>", ce.getVendaMedia()));
