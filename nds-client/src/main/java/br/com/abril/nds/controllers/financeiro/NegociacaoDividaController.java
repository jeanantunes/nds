...
public class NegociacaoDividaController {
	...
	public void index() {
		...
		Integer qntdParcelas = distribuidorService.obter()
				.getNegociacaoAteParcelas();
	...
	@Path("/pesquisar.json")
	public void pesquisar(FiltroConsultaNegociacaoDivida filtro,
			String sortname, String sortorder, int rp, int page) {

		this.session.setAttribute(FILTRO_NEGOCIACAO_DIVIDA, filtro);

		List<NegociacaoDividaDTO> list = negociacaoDividaService
				.obterDividasPorCota(filtro);
		...
		result.use(FlexiGridJson.class).from(listDividas)
				.total(listDividas.size()).page(page).serialize();
	}

	@Path("/pesquisarDetalhes.json")
	public void pesquisarDetalhes(FiltroConsultaNegociacaoDivida filtro,
			String sortname, String sortorder, int rp, int page) {
		// TODO
		this.session.setAttribute(FILTRO_NEGOCIACAO_DIVIDA, filtro);
		...
		/*
		 * for (Cobranca c : list){ NegociacaoDividaDetalheVO ndd = new
		 * NegociacaoDividaDetalheVO();
		 * ndd.setData(DateUtil.formatarDataPTBR(c.getDivida().getData()));
		 * if(c.getStatusCobranca() == StatusCobranca.PAGO)
		 * ndd.setTipo("Pagamento"); else ndd.setTipo("Dívida");
		 * ndd.setValor("-"+
		 * CurrencyUtil.formatarValor(c.getDivida().getValor()));
		 * ndd.setObservacao("TESTE"); listDividas.add(ndd); }
		 */
		result.use(FlexiGridJson.class).from(listDividas)
				.total(listDividas.size()).page(page).serialize();
	}
	...
	public void calcularParcelas(FiltroCalculaParcelas filtro) {
		 ...
		 * System.
		 * out.println("TipoPagamento "+filtro.getTipoPagamento().toString());
		 */
		...
		if (filtro.getTipoPagamento().equals(TipoCobranca.CHEQUE)) {
			...
			for (int i = 0; i < filtro.getQntdParcelas(); i++) {
				...
				/* VALOR DA PARCELA */
				Double valor = Double.valueOf((filtro.getValorSelecionado()
				...
				case DIARIO:
					parcela.setDataVencimento(DateUtil
							.formatarDataPTBR(DateUtil.adicionarDias(
									new Date(), 1)));
					break;
				...
				case MENSAL:
					...
					if (data.get(Calendar.DAY_OF_MONTH) > Integer
							.parseInt(filtro.getMensalDia())) {
						data.add(Calendar.MONTH, 1);
					}
					data.set(Calendar.DAY_OF_MONTH,
							Integer.parseInt(filtro.getMensalDia()));
					parcela.setDataVencimento(DateUtil.formatarDataPTBR(data
							.getTime()));
				 ...
				 * )){
				 * 
				 * parcela.setDataVencimento(DateUtil.formatarDataPTBR(DateUtil.
				 * adicionarDias(new Date(), 1)));
				 * 
				 * }else
				 * if(filtro.getPeriodicidade().equals(PeriodicidadeCobranca
				 * .SEMANAL)){
				 * 
				 ...
				 * adicionarDias(new Date(), 1)));
				 * 
				 * }else{ Calendar data = Calendar.getInstance();
				 * if(data.get(Calendar.DAY_OF_MONTH) >
				 * Integer.parseInt(filtro.getMensalDia())){
				 * data.add(Calendar.MONTH, 1); }
				 * data.set(Calendar.DAY_OF_MONTH,
				 * Integer.parseInt(filtro.getMensalDia()));
				 * parcela.setDataVencimento
				 * (DateUtil.formatarDataPTBR(data.getTime())); }
				 */

				/* ENCARGOS */
				Banco banco = bancoService.obterBancoPorId(filtro.getIdBanco());
				Double encargos = 0.0;
				BigDecimal juros = cobrancaService
						.calcularJuros(banco, cotaService
								.obterPorNumeroDaCota(filtro.getNumeroCota()),
								distribuidorService.obter(), BigDecimal
										.valueOf(valor), DateUtil
										.parseDataPTBR(parcela
												.getDataVencimento()),
								new Date());
				BigDecimal multas = cobrancaService
						.calcularMulta(banco, cotaService
								.obterPorNumeroDaCota(filtro.getNumeroCota()),
								distribuidorService.obter(), BigDecimal
										.valueOf(valor));
		...
		this.result.use(Results.json()).from(listParcelas, "result")
				.recursive().serialize();
	}
	...
	public void exportar(FileType fileType) throws IOException {
		...
		FiltroConsultaNegociacaoDivida filtro = (FiltroConsultaNegociacaoDivida) this.session
				.getAttribute(FILTRO_NEGOCIACAO_DIVIDA);

		List<NegociacaoDividaDTO> listDividas = negociacaoDividaService
				.obterDividasPorCota(filtro);

		FileExporter.to("consulta-box", fileType).inHTTPResponse(
				this.getNDSFileHeader(), null, null, listDividas,
				NegociacaoDividaDTO.class, this.httpServletResponse);
