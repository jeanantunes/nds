...
-- SET GLOBAL max_heap_table_size = 18446744073709551615;
-- SET GLOBAL tmp_table_size = 18446744073709551615;
...
select cod_produto,num_edicao,preco,QTDE_REPARTE_HVCT,QTDE_ENCALHE_HVCT,STATUS,CONSIGNADO,DEVOLVE_ENCALHE
from HVND where produto_edicao_id is null and STATUS = 1;

select distinct cod_produto,num_edicao, consignado
from HVND where produto_edicao_id is null and status = 1;
...
sum(qtde_reparte_hvct - qtde_encalhe_hvct) from hvnd  where consignado = 1
group by concat(
cod_produto,
num_edicao -- ,
-- COD_COTA_HVCT
...
h.data_recolhimento,
date(sysdate()),
case when h.status='F' then 'FECHADO' else 'EXPEDIDO' end,
'LANCAMENTO',
h.produto_edicao_id,
...
'LANCAMENTO',
HVND.produto_edicao_id
from HVND 
where HVND.produto_edicao_id not in (select lancamento.produto_edicao_id from lancamento)
and PARCIAL_NORMAL <> 'P'
group by produto_edicao_id,data_lancamento);
*/

delete from hvnd where produto_edicao_id = 0 or produto_edicao_id is null;
...
from HVND h
where not exists (
select 1 from lancamento l where h.produto_edicao_id = l.produto_edicao_id
and   h.data_lancamento = l.data_lcto_distribuidor)
and h.PARCIAL_NORMAL <> 'P'
and h.produto_edicao_id <> 0
group by h.produto_edicao_id,h.data_lancamento);


/*
update lancamento l , hvnd h
set l.data_lcto_distribuidor = h.data_lancamento,
l.data_rec_distrib = h.data_recolhimento
where l.produto_edicao_id = h.produto_edicao_id
and l.periodo_lancamento_parcial_id is null;
*/

update hvnd h ,lancamento l , produto_edicao pe, produto p, periodo_lancamento_parcial plp
set l.status = 'EXPEDIDO'
where l.produto_edicao_id = pe.id
and plp.id = l.PERIODO_LANCAMENTO_PARCIAL_ID
and pe.produto_id = p.id 
and p.codigo = h.cod_produto
and pe.numero_edicao = h.num_edicao
and plp.NUMERO_PERIODO = 1
and h.status = 'A' 
and l.status <> 'EXPEDIDO'
and h.data_recolhimento = l.data_rec_distrib;


-- NUMERO_LANCAMENTO

DROP TABLE IF EXISTS numero_lancamento_aux;

create table numero_lancamento_aux(
id bigint,
produto_edicao_id bigint,
linha bigint,
...
set @i = 0; 
set @a = 0; 
insert into numero_lancamento_aux 
select 
id,
produto_edicao_id,
case when produto_edicao_id = @a then @a := produto_edicao_id else @i := @i+1 end as a,
case when produto_edicao_id <> @a then @a := produto_edicao_id+@i else @i := 1 end as b
from lancamento 
where produto_edicao_id is not null
order by produto_edicao_id,data_lcto_prevista;


update numero_lancamento_aux aux2 , lancamento l
set l.numero_lancamento = aux2.linha -(select aux1.linha from numero_lancamento_aux aux1 where aux1.produto_edicao_id = aux2.produto_edicao_id limit 1) +1
where l.id = aux2.id;


truncate table numero_lancamento_aux;
...
select 
id,
concat(produto_edicao_id,periodo_lancamento_parcial_id),
case when concat(produto_edicao_id,periodo_lancamento_parcial_id) = @a then @a := concat(produto_edicao_id,periodo_lancamento_parcial_id) else @i := @i+1 end as a,
case when concat(produto_edicao_id,periodo_lancamento_parcial_id) <> @a then @a := concat(produto_edicao_id,periodo_lancamento_parcial_id)+@i else @i := 1 end as b
from lancamento 
where produto_edicao_id is not null
and periodo_lancamento_parcial_id is not null
order by produto_edicao_id,periodo_lancamento_parcial_id,data_lcto_prevista;
...
drop table numero_lancamento_aux;

update lancamento set reparte = 0 where reparte is null;
...
data_real,null,22,1,false,cota_id,concat('CARGA INICIAL - ', desc_credito,' - ',motivo),valor_decimal,1 -- Default DINAP 
...
(select true,data_real,'CARGA','APROVADO',data_real,data_real,null,23,1,false,cota_id,concat('CARGA INICIAL - ', desc_debito,' - ',motivo),valor_decimal,1
...
-- FIXME VERIFICAR O ARQUIVO


insert into estoque_produto_memoria (QTDE, QTDE_DEVOLUCAO_ENCALHE, QTDE_SUPLEMENTAR, PRODUTO_EDICAO_ID)
select 
sum(case when box in (150) then quantidade else 0 end) as QTDE,  -- 150 Rio, 60 Canpinas 
sum(case when box in (181) then quantidade else 0 end) as QTDE_DEVOLUCAO_ENCALHE, -- 181 Rio, 70 Campinas
sum(case when box in(140,142) then quantidade else 0 end) as QTDE_SUPLEMENTAR, -- (140,142) Rio, 80 Campinas
produto_edicao_id 
from ESTQBOX 
where box not in (191) -- 191=Rio,Santos / 92=Campinas 
group by 4;
...
(select distinct produto_edicao_id from ESTQBOX where box = 191 -- Rio 191, Campinas 92 FIXME Alterar para o box correspondente
and produto_edicao_id not in (select produto_edicao_id from estoque_produto_memoria));

select 'ESTOQUE_PRODUTO: ',sysdate(); -- Log


-- Validação que temos que fazer para eliminar da carga produtos do DISTRIB que passou do prazo de recolhimento 
-- e o distribuidor não recolheu
...
-- Cesar - rever isso
-- Insere os moviemento de devolução para o fornecedor baseado no encalhe das cotas Com exceção dos produtos que ainda estão do distribuidor 
-- (Esses produto serão excluídos após rollaut)
INSERT INTO movimento_estoque_memoria 
(DATA,TIPO_MOVIMENTO_ID,QTDE,PRODUTO_EDICAO_ID,ESTOQUE_PRODUTO_ID)
(select 
me.data,
...
/*COMENTADO EM ACORDO COM O CESAR
...
and data_rec_distrib > DATE_SUB(sysdate(),INTERVAL 5 DAY) -- 5 dias menor que a carga   
and status = 'EM_RECOLHIMENTO'
...
update lancamento set data_lcto_prevista = '0001-01-01' where data_lcto_prevista = '0000-00-00';
...
and l.DATA_REC_PREVISTA < DATE_SUB(sysdate(),INTERVAL 7 DAY);
...
/* COMENTADO ATRAVES DE SOLICITAÇÂO DO CESAR.
insert into chamada_encalhe_lancamento
select a.id, b.id 
from chamada_encalhe a, lancamento b
where a.produto_edicao_id = b.produto_edicao_id
and status = 'EM_RECOLHIMENTO'
and b.id not in (select lancamento_id from chamada_encalhe_lancamento);
*/
...
select 'CHAMADA_ENCALHE_LANCAMENTO: ',sysdate(); -- Log

/*
insert into nota_envio_endereco (BAIRRO, CEP, CIDADE, CODIGO_CIDADE_IBGE, CODIGO_UF, COMPLEMENTO, ID, LOGRADOURO, NUMERO, TIPO_LOGRADOURO, UF)
select BAIRRO, CEP, CIDADE, CODIGO_CIDADE_IBGE, CODIGO_UF, COMPLEMENTO, ID, LOGRADOURO, NUMERO, TIPO_LOGRADOURO, UF
from endereco e
where e.id in (
	select ENDERECO_ID_DESTINATARIO 
	from nota_envio ne
);

SET foreign_key_checks = 0;
ALTER TABLE nota_envio 
ADD CONSTRAINT fk_ne_nota_envio_endereco FOREIGN KEY (ENDERECO_ID_DESTINATARIO) REFERENCES nota_envio_endereco (ID) ON DELETE NO ACTION ON UPDATE NO ACTION;
SET foreign_key_checks = 1;
*/

-- select 'NOTA_ENVIO_ENDERECO: ',sysdate(); -- Log

DROP TABLE IF EXISTS estoque_produto_memoria;
DROP TABLE IF EXISTS movimento_estoque_cota_memoria;
DROP TABLE IF EXISTS movimento_estoque_memoria;

DROP TABLE IF EXISTS estoque_produto_memoria_aux;
DROP TABLE IF EXISTS movimento_estoque_cota_memoria_aux;
DROP TABLE IF EXISTS movimento_estoque_memoria_aux;
