...
--
-- ****** VERIFICAR POR QUE EXISTE CODIGO< EDICAO DUPLICADOS
update CARGA_LANCAMENTO_MDC set produto_edicao_id = 
 (select pe.id 
  ...
  and pe.numero_edicao = CARGA_LANCAMENTO_MDC.num_edicao limit 1);
...
order by STR_TO_DATE(data_real_lancamento_lanp,'%d/%m/%Y') asc limit 1), 
...
-- 
-- **** CONFIRMAR SEMPRE QUAL OS NUMEROS DE BOX DO DISTRIBUIDOR
--
...
--
-- *** ALTERAR PARA O BOX CORRETO DO DISTRIBUIDOR
--
...
update movimento_estoque_cota set valor_Desconto = 30; -- validar se desconto DF é 25
