...
echo
echo '10) CARREGA PRODIN MDC.' `date +%T` $DIR/comum/carga_prodin_mdc_agencia.sql
mysql -h$AMBIENTE -u$DBUSER -p$DBPASS $BASE < $DIR/comum/carga_prodin_mdc_agencia.sql
echo
echo '11) CARREGA MOVIMENTAÇÕES.' `date +%T` $DIR/comum/carga_movimentos_parcial_sao_paulo_agencia.sql
...
#mysqldump -h$AMBIENTE -u$DBUSER -p$DBPASS --single-transaction --routines --triggers db_05318019 | sed -e '/^\/\*\!50013 DEFINER/d' | sed -e 's/\/\*\!50017 DEFINER\=`root`@`%`\*\/ //g' | sed 's/DEFINER\=`root`@`%`//g' | sed -e 's/\/\*\!50017 DEFINER\=`root`@`localhost`\*\/ //g' | sed 's/DEFINER\=`root`@`localhost`//g' > $DIRBKP/db_05318019.sql
echo
echo '13) SCP PARA DEVWEB.' `date +%T`
#scp -i /home/dguerra/.ssh/kp-hom-distb.pem -o "StrictHostKeyChecking no" $DIRBKP/db_05318019.sql douglas@10.129.28.111:/home/douglas
echo
echo '14) Publica base AGENCIA RIO no RDS de Producao.' `date +%T`
#ssh -i /home/dguerra/.ssh/kp-hom-distb.pem -o "StrictHostKeyChecking no" douglas@10.129.28.111 "/home/douglas/publica_dump.sh db_05318019"
echo
echo 'Fim.' `date`
