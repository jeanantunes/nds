select * from hvnd_mdc;

-- drop table hvnd_mdc;

ALTER TABLE HVND_MDC
ADD COLUMN cod_produto VARCHAR(10);


update HVND_MDC 
set cod_produto = lpad(cod_produto_hvct,8,'0');


update hvnd_mdc
set cod_produto = '1000050501' 
where cod_produto = '00050501';


update hvnd_mdc
set cod_produto = '1000060501' 
where cod_produto = '00060501';
...
and pe.numero_edicao = h.num_edicao; -- 713518
...
(select 
sum(csv.QTDE_REPARTE_HVCT-QTDE_VENDA_HVCT) QTDE_DEVOLVIDA,
...
h.data_recolhimento,
26,
h.QTDE_REPARTE_HVCT - h.QTDE_VENDA_HVCT, 
...
INSERT INTO estoque_produto_cota (ID,QTDE_DEVOLVIDA, QTDE_RECEBIDA, COTA_ID, PRODUTO_EDICAO_ID)
( select
m.ID,m.QTDE_DEVOLVIDA,m.QTDE_RECEBIDA,m.COTA_ID,m.PRODUTO_EDICAO_ID
from estoque_produto_cota_memoria m,estoque_produto_cota e
where (m.produto_edicao_id <> e.produto_edicao_id and m.cota_id <> e.cota_id));
*/
...
drop table movimento_estoque_cota_memoria_aux;
...
drop table movimento_estoque_memoria_aux;
...
drop table chamada_encalhe_aux;
...
drop table chamada_encalhe_cota_aux;
...
drop table controle_conferencia_encalhe_aux;
...
drop table controle_conferencia_encalhe_cota_aux;
...
select
mec.id as mecid,
mec.produto_edicao_id produtoEdicaoId,
cast(sum( case when tm.OPERACAO_ESTOQUE = 'ENTRADA' then mec.QTDE else -mec.QTDE end ) as unsigned int) AS reparte,
(case when l.status  IN ('FECHADO','RECOLHIDO','EM_RECOLHIMENTO')
then cast(sum(CASE WHEN tm.OPERACAO_ESTOQUE = 'ENTRADA'
 THEN mec.QTDE ELSE -mec.QTDE                  
END) - (select sum(mecEncalhe.qtde)                      
from lancamento lanc                      
LEFT JOIN chamada_encalhe_lancamento cel on cel.LANCAMENTO_ID = lanc.ID                      
LEFT JOIN chamada_encalhe ce on ce.id = cel.CHAMADA_ENCALHE_ID                      
LEFT JOIN chamada_encalhe_cota cec on cec.CHAMADA_ENCALHE_ID = ce.ID                      
LEFT JOIN cota cota on cota.id = cec.COTA_ID                      
LEFT JOIN conferencia_encalhe confEnc on confEnc.CHAMADA_ENCALHE_COTA_ID = cec.ID                      
LEFT JOIN movimento_estoque_cota mecEncalhe on mecEncalhe.id = confEnc.MOVIMENTO_ESTOQUE_COTA_ID                   
WHERE lanc.id = l.id and cota.id = c.id) AS UNSIGNED INT)             
else null end) as venda  
from movimento_estoque_cota mec force index (NDX_PRODUTO_EDICAO)  
inner join tipo_movimento tm on tm.id = mec.tipo_movimento_id  
inner join cota c on c.id = mec.COTA_ID  
inner join
lancamento l on mec.lancamento_id = l.id  
left outer join periodo_lancamento_parcial plp on plp.id = l.PERIODO_LANCAMENTO_PARCIAL_ID  
where 1 = 1 
and c.NUMERO_COTA = 8 
and mec.PRODUTO_EDICAO_ID in (
18 -- 36093, 36092, 36091, 36090, 18
) 
and l.STATUS IN (
'EXPEDIDO', 'EM_BALANCEAMENTO_RECOLHIMENTO', 'BALANCEADO_RECOLHIMENTO', 'EM_RECOLHIMENTO', 'RECOLHIDO', 'FECHADO'
) 
and tm.GRUPO_MOVIMENTO_ESTOQUE  <> 'ENVIO_ENCALHE'  
