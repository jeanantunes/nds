...
public class UtilServer extends HttpServlet {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(UtilServer.class);
	/**
				...
				for (File conf : dir.listFiles()) {
					if (conf.isFile() && conf.getName().endsWith(".conf")) {
						...
						try {
							...
							LOGGER.error("Nao leu os dados do arquivo -> " + conf.getName(), ex);
						} finally {
	...
	public static byte[] getPDF(byte[] obj, String formato) {
		...
		try {
			...
			LOGGER.error("Nao salvaou o html.", ex);
			obj = null;
		}
		...
		try {
			...
			LOGGER.error("Erro na leitura do pdf.", ex);
			obj = null;
		}
	...
	public static String getPDF(String pathHtml, String formato) {
		...
		try {
			...
			LOGGER.error("Erro na geracao do pdf.", ex);
			return null;
		}
	...
	public static byte[] getZIP(Map<String, byte[]> arquivos) {
		...
		try {
			...
			LOGGER.error("Nao recuperou o zip.", ex);
			return null;
		} finally {
			try {
				zout.close();
	...
	public static SortedMap<String, byte[]> getArquivos(byte[] zip) {
		...
		try {
			...
			LOGGER.error("Nao pegou os arquivos", ex);
			return null;
		}
	...
	public static String getXml(Node node) {
		...
		try {
			...
			LOGGER.error("Erro ao formatar em string o doc.", e);
			return null;
		}
	}
	...
	public static Document getXml(String xml, String xsd, ErrorHandler error) {
		...
		try {
			...
			LOGGER.error("Erro ao formatar a string em xml.", e);
			return null;
		}
	}
	...
	public static String getValorTag(Element ele, String tag, boolean excecao) throws NullPointerException {
		try {
			...
			if (excecao) {
				LOGGER.debug("Nao achou a tag -> " + tag);
	 ...
	 *             exceção disparada em caso de erro.
	 */
	public static String getTextoArquivo(String pathArquivo) throws Exception {
		try {
