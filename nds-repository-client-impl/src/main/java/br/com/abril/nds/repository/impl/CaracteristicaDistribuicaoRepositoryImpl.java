...
CaracteristicaDistribuicaoRepository {
    ...
    public List<CaracteristicaDistribuicaoDTO> obterCaracteristicaDistribuicaoDetalhe(
        ...
        .append(" (select sum(mec.qtde)                                         ")
		.append(" from movimento_estoque_cota mec                               ")
		.append(" inner join tipo_movimento tm on tm.id = mec.tipo_movimento_id ")
		.append(" where tm.grupo_movimento_estoque = 'RECEBIMENTO_REPARTE'      ")
		.append(" and mec.produto_edicao_id = ped.id) as 'reparte', 			")
		.append(" (select sum(case when (tm.grupo_movimento_estoque = 'DEVOLUCAO_ENCALHE') then -mec.qtde else mec.qtde end) ")
		.append(" from movimento_estoque_cota mec                               ")
		.append(" inner join tipo_movimento tm on tm.id = mec.tipo_movimento_id ")
		.append(" where tm.grupo_movimento_estoque = 'DEVOLUCAO_ENCALHE'        ")
		.append(" and mec.produto_edicao_id = ped.id) as 'venda',   			")
