...
@Repository
public class ChamadaEncalheCotaRepositoryImpl extends AbstractRepositoryModel<ChamadaEncalheCota, Long>
		implements ChamadaEncalheCotaRepository {
	...
	@Override
	public BigDecimal obterReparteDaChamaEncalheCota(Integer numeroCota, List<Date> datas, Boolean conferido,
			Boolean postergado) {
	...
	@Override
	public BigDecimal obterTotalDescontoDaChamaEncalheCota(Integer numeroCota, Date dataOperacao, Boolean conferido,
			Boolean postergado) {
	...
	@Override
	public BigDecimal obterTotalDaChamaEncalheCotaSemDesconto(Integer numeroCota, List<Date> datasOperacao,
	...
	private String getSqlValor(Boolean conferido, Boolean postergado, String valor) {
		...
		if (REPARTE_COM_DESCONTO.equals(valor)) {
			sqlValor.append(
					" COALESCE( PRECOS_DE_REPARTE.PRECO_COM_DESCONTO, PROD_EDICAO.PRECO_VENDA ) * CH_ENCALHE_COTA.QTDE_PREVISTA ");
			...
			sqlValor.append(
					" COALESCE ( PRECOS_DE_REPARTE.PRECO_VENDA, PROD_EDICAO.PRECO_VENDA ) * CH_ENCALHE_COTA.QTDE_PREVISTA ");
		}
		...
		// O VALOR DE PRECO COM DESCONTO MAIS ATUAL DESTE APLICADO NO MOV. EST.
		// COTA DE REPARTE.
		...
		sql.append(
				" 		INNER JOIN (	SELECT                                                                                       ");
		sql.append(
				" 							MEC.PRODUTO_EDICAO_ID AS PRODUTO_EDICAO_ID,                                              ");
		sql.append(
				" 							MEC.COTA_ID, MAX(MEC.DATA) AS DATA                                                       ");
		sql.append(
				" 						FROM                                                                                         ");
		sql.append(
				" 							CHAMADA_ENCALHE_COTA                                                                     ");
		sql.append(
				" 						INNER JOIN CHAMADA_ENCALHE ON (CHAMADA_ENCALHE.ID = CHAMADA_ENCALHE_COTA.CHAMADA_ENCALHE_ID) ");
		sql.append(
				" 						INNER JOIN MOVIMENTO_ESTOQUE_COTA MEC ON (                                                   ");
		sql.append(
				" 								CHAMADA_ENCALHE_COTA.COTA_ID = MEC.COTA_ID AND                                       ");
		sql.append(
				" 								CHAMADA_ENCALHE.PRODUTO_EDICAO_ID = MEC.PRODUTO_EDICAO_ID)                           ");

		sql.append(
				" 						INNER JOIN TIPO_MOVIMENTO ON ( TIPO_MOVIMENTO.ID = MEC.TIPO_MOVIMENTO_ID )");
		// sql.append(" AND TIPO_MOVIMENTO.GRUPO_MOVIMENTO_ESTOQUE in
		// (:grupoMovimentoEnvioReparte) ) ");

		sql.append(" 						WHERE ");
		sql.append(
				" 							CHAMADA_ENCALHE_COTA.COTA_ID = (SELECT ID FROM COTA WHERE NUMERO_COTA = :numeroCota) AND ");
		...
		sql.append(
				" 		ON (                                                                                                         ");
		sql.append(
				" 			MEC.PRODUTO_EDICAO_ID = MOVCOTA.PRODUTO_EDICAO_ID AND                                                    ");
		...
		// sql.append(" INNER JOIN TIPO_MOVIMENTO ON ( TIPO_MOVIMENTO.ID =
		// MEC.TIPO_MOVIMENTO_ID AND ");
		// sql.append(" TIPO_MOVIMENTO.GRUPO_MOVIMENTO_ESTOQUE in
		// (:grupoMovimentoEnvioReparte) ) ");
		sql.append(
		...
		sql.append(
				" 	)                                                                                                                ");
		sql.append(
				" AS PRECOS_DE_REPARTE ON (CH_ENCALHE.PRODUTO_EDICAO_ID = PRECOS_DE_REPARTE.PRODUTO_EDICAO_ID)                        ");
	...
	public List<Long> obterListaCotaChamadao(Date dataOperacao) {
		...
		hql.append(
				" inner join chamada_encalhe ce USE index ( NDX_CHAMADA_ENCALHE ) on ce.id = cec.chamada_encalhe_id  ");
	...
	public Long obterQtdListaChamaEncalheCota(Integer numeroCota, Date dataOperacao, Long idProdutoEdicao,
	...
	public ChamadaEncalheCota obterUltimaChamaEncalheCota(Cota cota, Long idProdutoEdicao, boolean postergado,
			Date dataOperacao) {
	...
	@Override
	public Date obterDataChamadaEncalheCotaProximaDataOperacao(Cota cota, Long idProdutoEdicao, boolean postergado,
			Date dataOperacao) {
	...
	public ChamadaEncalheCota obterChamadaEncalheCotaNaData(Cota cota, Long idProdutoEdicao, boolean postergado,
			Date dataOperacao) {
	...
	public ChamadaEncalheCota buscarPorChamadaEncalheECota(Long idChamadaEncalhe, Long idCota) {
		...
		hql.append(" select o from ChamadaEncalheCota o ").append(" WHERE o.cota.id =:idCota ")
	...
	public List<ChamadaAntecipadaEncalheDTO> obterCotasProgramadaParaAntecipacoEncalhe(
		 ...
		 * PDV associado a mesma cota.
		 */
		hql.append("SELECT DISTINCT new ").append(ChamadaAntecipadaEncalheDTO.class.getCanonicalName())
				.append(" (box.codigo, ").append(" box.nome, ").append(" cota.numeroCota, ")
				.append(" chamadaEncalheCota.qtdePrevista,").append(" lancamento.id, ")
				...
				.append(" chamadaEncalheCota.id,").append(" chamadaEncalhe.tipoChamadaEncalhe, ")
				.append(" chamadaEncalhe.dataRecolhimento, ").append(" lancamento.dataRecolhimentoDistribuidor, ")
				.append(" lancamento.dataRecolhimentoPrevista ").append(" ) ");
	...
	private Object getOrderByCotasProgramadaParaAntecipacaoEncalhe(FiltroChamadaAntecipadaEncalheDTO filtro) {
		...
		case BOX:
			...
			break;
		case NOME_COTA:
			hql.append(" order by   ").append(" case when (pessoa.nome is not null) then ( pessoa.nome )")
	...
	private Object getSqlFromEWhereCotasProgramadaParaAntecipacaoEncalhe(FiltroChamadaAntecipadaEncalheDTO filtro) {
		...
		hql.append("  FROM ChamadaEncalheCota chamadaEncalheCota ")
				...
				.append(" JOIN produtoEdicao.lancamentos lancamento ").append(" JOIN produtoEdicao.produto produto ")
				.append(" JOIN chamadaEncalheCota.cota cota ").append(" JOIN cota.pessoa pessoa ")
				.append(" JOIN cota.box box ").append(" JOIN cota.pdvs pdv ").append(" LEFT JOIN pdv.rotas rotaPdv  ")
				.append(" LEFT JOIN rotaPdv.rota rota  ").append(" LEFT JOIN rota.roteiro roteiro ");
		...
		hql.append(" WHERE ").append(" chamadaEncalhe.tipoChamadaEncalhe=:tipoChamadaEncalhe")
	...
	public List<ChamadaEncalheCotaDTO> buscarPorChamadaEncalhe(ChamadaEncalhe ce) {
		...
		sql.append(" select cec.id as id, cec.CHAMADA_ENCALHE_ID as numeroChamadaEncalhe, ")
				...
				.append(" from chamada_encalhe_cota cec ").append(" inner join cota c on c.id = cec.COTA_ID ")
	...
	public void removerChamadaEncalheCotaZerada(Date dataConfirmada) {
		...
		query.setParameter("dataConfirmada", dataConfirmada);
	...
	public List<ProdutoEdicaoECota> obterListaChamadaoByListLancamento(Set<Long> idLancamento, Date dataOperacao) {

		StringBuilder hql = new StringBuilder();

		hql.append(" select l.produto_edicao_id as produtoEdicaoID, cec.cota_id as cotaID from chamada_encalhe_cota cec ");

		hql.append("  inner join chamada_encalhe ce on ce.id = cec.chamada_encalhe_id ");
		
		hql.append("  inner join chamada_encalhe_lancamento cel on ce.id = cel.chamada_encalhe_id ");
		
		hql.append("  inner join lancamento l on l.id = cel.lancamento_id ");
		...
		query.setResultTransformer(new AliasToBeanResultTransformer(ProdutoEdicaoECota.class));
