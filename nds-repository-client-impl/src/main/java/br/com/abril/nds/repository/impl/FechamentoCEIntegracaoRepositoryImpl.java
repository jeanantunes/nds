...
public class FechamentoCEIntegracaoRepositoryImpl extends AbstractRepositoryModel<FechamentoEncalhe, FechamentoEncalhePK> implements FechamentoCEIntegracaoRepository {
	...
	public List<ItemFechamentoCEIntegracaoDTO> buscarItensFechamentoSemCEIntegracao(FiltroFechamentoCEIntegracaoDTO filtro) {
		...
		hql.append("  + COALESCE(ESTOQUE_PROD.QTDE_SUPLEMENTAR, 0)  ");
		hql.append("  + COALESCE(ESTOQUE_PROD.QTDE_GANHO, 0) - ESTOQUE_PROD.QTDE_PERDA) AS qtdDevolucao, ");
		hql.append("  COALESCE(PROD_EDICAO.PRECO_VENDA - (PROD_EDICAO.PRECO_VENDA * PROD_EDICAO.DESCONTO / 100) , 0) * (COALESCE(ESTOQUE_PROD.QTDE_DEVOLUCAO_ENCALHE, 0) + COALESCE(ESTOQUE_PROD.QTDE, 0) + COALESCE(ESTOQUE_PROD.QTDE_SUPLEMENTAR, 0) - ESTOQUE_PROD.QTDE_PERDA)  AS valorTotalComDesconto,");
		
		hql.append(" (COALESCE(ESTOQUE_PROD.QTDE_DEVOLUCAO_ENCALHE, 0) + COALESCE(ESTOQUE_PROD.QTDE, 0) + COALESCE(ESTOQUE_PROD.QTDE_SUPLEMENTAR, 0) - ESTOQUE_PROD.QTDE_PERDA) * COALESCE(PROD_EDICAO.PRECO_VENDA, 0) AS valorTotal, ");
