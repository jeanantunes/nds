...
@Repository
public class MovimentoEstoqueCotaRepositoryImpl extends AbstractRepositoryModel<MovimentoEstoqueCota, Long>
		implements MovimentoEstoqueCotaRepository {
	...
	public List<MovimentoEstoqueCotaGenericoDTO> obterListaMovimentoEstoqueCotaDevolucaoJuramentada(
	 ...
	 * (non-Javadoc)
	 * 
	 * @see br.com.abril.nds.repository.MovimentoEstoqueCotaRepository#
	 * obterListaMovimentoEstoqueCotaParaOperacaoConferenciaEncalhe(java.lang.
	 * Long)
	 */
	@Override
	@SuppressWarnings("unchecked")
	public List<MovimentosEstoqueEncalheDTO> obterListaMovimentoEstoqueCotaParaOperacaoConferenciaEncalhe(
		...
		hql.append(
				"INNER JOIN MOVIMENTO_ESTOQUE_COTA movimentoe1_ ON conferenci0_.MOVIMENTO_ESTOQUE_COTA_ID=movimentoe1_.ID ");
		...
		final Query query = getSession().createSQLQuery(hql.toString())
				.addScalar("idMovimentoEstoqueCota", LongType.INSTANCE).addScalar("idCota", LongType.INSTANCE)
				.addScalar("idProdutoEdicao", LongType.INSTANCE).addScalar("idFornecedor", LongType.INSTANCE)

				.addScalar("qtde", BigIntegerType.INSTANCE).addScalar("precoComDesconto", BigDecimalType.INSTANCE)
				.addScalar("precoVenda", BigDecimalType.INSTANCE).addScalar("valorDesconto", BigDecimalType.INSTANCE);
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<MovimentoEstoqueCota> obterMovimentosAVistaPendentesGerarFinanceiro(final Long idCota,
		...
		query.setParameterList("gruposMovimentoReparte", Arrays.asList(GrupoMovimentoEstoque.COMPRA_SUPLEMENTAR,
				GrupoMovimentoEstoque.COMPRA_ENCALHE, GrupoMovimentoEstoque.RECEBIMENTO_REPARTE));
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<MovimentoEstoqueCota> obterMovimentosConsignadosCotaAVistaPrevistoDia(final Long idCota,
			final Date dataLancamento) {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<MovimentosEstoqueEncalheDTO> obterMovimentosPendentesGerarFinanceiroComChamadaEncalheOuProdutoContaFirme(
			final Long idCota, final List<Date> datas, List<Long> idTiposMovimentoEstoque) {
		...
		sql.append(
				"INNER JOIN chamada_encalhe_cota cec on cec.CHAMADA_ENCALHE_ID = ce.id and cec.cota_id = mec.cota_id ");
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<MovimentoEstoqueCota> obterMovimentosEstornadosPorChamadaEncalhe(final Long idCota,
			final List<Long> idsTipoMovimentoEstorno, List<Date> datas) {
		...
		sql.append(
				" inner join chamada_encalhe ce on ce.id = cec.chamada_encalhe_id and ce.produto_edicao_id = mec.produto_edicao_id ");
		sql.append(" inner join chamada_encalhe_lancamento cel on cel.CHAMADA_ENCALHE_ID = ce.id ");
		...
		// sql.append(" and mec.STATUS_ESTOQUE_FINANCEIRO =
	 ...
	 * 
	 * @see br.com.abril.nds.repository.MovimentoEstoqueCotaRepository#
	 * obterQtdeMovimentoEstoqueCotaParaProdutoEdicaoNoPeriodo(java.lang.Long,
	 * java.lang.Long, java.util.Date, java.util.Date,
	 * br.com.abril.nds.model.estoque.OperacaoEstoque)
	 */
	@Override
	public BigInteger obterQtdeMovimentoEstoqueCotaParaProdutoEdicaoNoPeriodo(final Long idCota,
			final Long idProdutoEdicao, final Date dataInicial, final Date dataFinal,
	 ...
	 */
	@Override
	public BigDecimal obterValorTotalMovimentoEstoqueCotaParaProdutoEdicaoNoPeriodo(final Long idCota,
			final Long idFornecedor, final Long idProdutoEdicao, final Date dataInicial, final Date dataFinal,
			final OperacaoEstoque operacaoEstoque) {
	...
	private StringBuilder obterQueryListaConsultaEncalhe(final FiltroConsultaEncalheDTO filtro,
		...
		if (filtro.getIdCota() != null) {
			...
			// subSqlVendaProduto.append(" and vp.TIPO_COMERCIALIZACAO_VENDA <>
			// 'CONTA_FIRME' ");
		}

		final StringBuilder qtdeInformadaEncalhe = new StringBuilder(
				"coalesce(sum(COALESCE(CONFERENCIA_ENCALHE.QTDE, 0)), 0)");
		...
		if (filtro.getIdCota() != null) {
			...
			subSqlValoresDesconto.append(
					"  COALESCE(TBL_PRECO_VENDA.PRECO_COM_DESCONTO, PRODUTO_EDICAO.PRECO_VENDA, 0) as precoComDesconto, ");
			subSqlValoresDesconto.append("	COALESCE(TBL_PRECO_VENDA.VALOR_DESCONTO, 0) as valorDesconto, ");
			subSqlValoresDesconto.append("	( ( " + qtdeInformadaEncalhe
					+ ") * COALESCE(TBL_PRECO_VENDA.PRECO_COM_DESCONTO , PRODUTO_EDICAO.PRECO_VENDA, 0) ) as valorComDesconto, ");
			...
			subSqlValoresDesconto.append("   COALESCE(PRODUTO_EDICAO.PRECO_VENDA, 0) - ");
			subSqlValoresDesconto.append(
					"   (COALESCE(PRODUTO_EDICAO.PRECO_VENDA, 0) * COALESCE(PRODUTO_EDICAO.DESCONTO, PRODUTO.DESCONTO, 0) / 100) ");
			...
			subSqlValoresDesconto.append(
					"  COALESCE(( " + qtdeInformadaEncalhe + " ), 0) * COALESCE(PRODUTO_EDICAO.PRECO_VENDA, 0) - ");
			subSqlValoresDesconto.append(
					"  (COALESCE(( " + qtdeInformadaEncalhe + " ), 0) * COALESCE(PRODUTO_EDICAO.PRECO_VENDA, 0) * ");
			subSqlValoresDesconto.append("  COALESCE(DESCONTO_LOGISTICA.PERCENTUAL_DESCONTO, 0) / 100 )   ");
		...
		if (filtro.getIdCota() != null) {
			subSqlIndObservacao.append(
					" INNER JOIN CHAMADA_ENCALHE_COTA on CONFERENCIA_ENCALHE_0.CHAMADA_ENCALHE_COTA_ID = CHAMADA_ENCALHE_COTA.id ");
		}
		...
		subSqlIndObservacao.append(
				" 	and (CONFERENCIA_ENCALHE_0.OBSERVACAO is not null OR CONFERENCIA_ENCALHE_0.JURAMENTADA = 1) ");
		...
		if (counting) {
			...
			if (filtro.getIdCota() != null) {
				sql.append("	( ( " + qtdeInformadaEncalhe
						+ ") * COALESCE(TBL_PRECO_VENDA.PRECO_VENDA, PRODUTO_EDICAO.PRECO_VENDA, 0) ) as valor, ");
			} else {
				sql.append(
						"	( ( " + qtdeInformadaEncalhe + " ) * COALESCE(PRODUTO_EDICAO.PRECO_VENDA, 0) ) as valor, ");
			}

			sql.append(" TBL_REPARTE.REPARTE as reparte, ");

			sql.append("( ( ").append(qtdeInformadaEncalhe).append(" ) - ( ").append(subSqlVendaProduto)
					.append(") ) as encalhe, ");
	...
	public Integer obterQtdeConsultaEncalhe(final FiltroConsultaEncalheDTO filtro) {
		...
		parameters.put("grupoMovimentoEstoqueConsignado",
				Arrays.asList(GrupoMovimentoEstoque.RECEBIMENTO_REPARTE.name(),
						GrupoMovimentoEstoque.COMPRA_ENCALHE.name(), GrupoMovimentoEstoque.COMPRA_SUPLEMENTAR.name()));
	...
	public BigDecimal obterSaldoEntradaNoConsignado(Date dataRecolhimento, TipoCota tipoCota) {
		...
		sql.append(" SELECT ");
		sql.append(
				"	SUM( IF(TM.OPERACAO_ESTOQUE='SAIDA', MEC.QTDE*-1, MEC.QTDE) ) AS QTDREPARTE,                              ");
		sql.append(
				"	COALESCE(MEC.PRECO_VENDA, PRODUTO_EDICAO.PRECO_VENDA, 0) AS PRECOVENDA,                                   ");
		sql.append(
				"	COALESCE(CONFERENCIA_ENCALHE.DIA_RECOLHIMENTO,1) AS DIARECOLHIMENTO                                       ");
		sql.append(
				" FROM                                                                                                          ");
		sql.append(
				"	CHAMADA_ENCALHE                                                                                           ");
		sql.append(
				"	INNER JOIN CHAMADA_ENCALHE_COTA ON ( CHAMADA_ENCALHE_COTA.CHAMADA_ENCALHE_ID = CHAMADA_ENCALHE.ID )       ");
		sql.append(
				"	INNER JOIN PRODUTO_EDICAO ON ( PRODUTO_EDICAO.ID = CHAMADA_ENCALHE.PRODUTO_EDICAO_ID )                    ");
		sql.append(
				"	INNER JOIN PRODUTO PRODUTO ON ( PRODUTO.ID = PRODUTO_EDICAO.PRODUTO_ID )                                  ");
		...
		sql.append(
				"	INNER JOIN CHAMADA_ENCALHE_LANCAMENTO cel on cel.CHAMADA_ENCALHE_ID = CHAMADA_ENCALHE.ID AND cel.LANCAMENTO_ID = MEC.LANCAMENTO_ID ");
	...
	public ConsultaEncalheDTO obterValorTotalReparteEncalheDataCotaFornecedor(final FiltroConsultaEncalheDTO filtro) {
		...
		return (ConsultaEncalheDTO) namedParameterJdbcTemplate.queryForObject(sql.toString(), parameters,
				cotaRowMapper);
	}
	...
	public BigDecimal obterValorTotalEncalhe(final FiltroConsultaEncalheDTO filtro) {
		...
		sql.append(
				"	SUM( MOVIMENTO_ESTOQUE_COTA.QTDE * COALESCE(MOVIMENTO_ESTOQUE_COTA.PRECO_COM_DESCONTO, PRODUTO_EDICAO.PRECO_VENDA, 0) ) as totalEncalhe ");

		sql.append("	from	");

		sql.append(
				"	CONTROLE_CONFERENCIA_ENCALHE_COTA CONTROLE_CONF_ENC_COTA, MOVIMENTO_ESTOQUE_COTA inner join CONFERENCIA_ENCALHE on ");
		...
		sql.append(
				"	AND (CHAMADA_ENCALHE.DATA_RECOLHIMENTO BETWEEN :dataRecolhimentoInicial AND :dataRecolhimentoFinal) ");
	...
	public StringBuilder getFromWhereConsultaEncalhe(final FiltroConsultaEncalheDTO filtro) {
		...
		sqlTblPrecoVenda.append(
				" INNER JOIN CONFERENCIA_ENCALHE ON (CONFERENCIA_ENCALHE.CONTROLE_CONFERENCIA_ENCALHE_COTA_ID = CCEC.ID) ");
		...
		sqlTblPrecoVenda
				.append(" INNER JOIN PRODUTO_EDICAO ON (PRODUTO_EDICAO.ID = CONFERENCIA_ENCALHE.PRODUTO_EDICAO_ID)  ");
		sqlTblPrecoVenda
				.append(" INNER JOIN PRODUTO ON (PRODUTO_EDICAO.PRODUTO_ID = PRODUTO.ID)                            ");
		sqlTblPrecoVenda
				.append(" INNER JOIN PRODUTO_FORNECEDOR ON (PRODUTO_FORNECEDOR.PRODUTO_ID = PRODUTO.ID)             ");
		sqlTblPrecoVenda
				.append(" INNER JOIN FORNECEDOR ON (PRODUTO_FORNECEDOR.FORNECEDORES_ID = FORNECEDOR.ID)             ");
		sqlTblPrecoVenda
				.append(" INNER JOIN PESSOA ON (PESSOA.ID = FORNECEDOR.JURIDICA_ID)                                 ");
		sqlTblPrecoVenda
				.append(" WHERE CCEC.DATA_OPERACAO BETWEEN :dataRecolhimentoInicial AND :dataRecolhimentoFinal		");
		...
		sqlTblPrecoVenda
				.append(filtro.getIdProdutoEdicao() != null ? " AND PRODUTO_EDICAO.ID = :idProdutoEdicao " : "");
		sqlTblPrecoVenda.append(filtro.getIdBoxEncalhe() != null ? " AND BOX.ID = :boxEncalhe " : "");
		sqlTblPrecoVenda
				.append(" GROUP BY CONFERENCIA_ENCALHE.PRODUTO_EDICAO_ID) AS EDICAO_ENCALHADA ON  					");
		...
		// sqlTblPrecoVenda.append(" WHERE
		// TIPO_MOVIMENTO.GRUPO_MOVIMENTO_ESTOQUE in
		// (:grupoMovimentoEstoqueConsignado) ");
		...
		sqlTblPrecoVenda.append(
				" ) AS PRECO_VENDA_DE_REPARTE ON (MOVIMENTO_ESTOQUE_COTA.PRODUTO_EDICAO_ID = PRECO_VENDA_DE_REPARTE.PRODUTO_EDICAO_ID AND ");
		sqlTblPrecoVenda.append(
				" 								MOVIMENTO_ESTOQUE_COTA.DATA = PRECO_VENDA_DE_REPARTE.DATA_REPARTE)                       ");
		sqlTblPrecoVenda.append(
				" INNER JOIN TIPO_MOVIMENTO ON (TIPO_MOVIMENTO.ID = MOVIMENTO_ESTOQUE_COTA.TIPO_MOVIMENTO_ID) ");
		sqlTblPrecoVenda.append(" WHERE 1=1 ");
		// sqlTblPrecoVenda.append(" WHERE
		// TIPO_MOVIMENTO.GRUPO_MOVIMENTO_ESTOQUE in
		// (:grupoMovimentoEstoqueConsignado) ");
		...
		sqlTblPrecoVenda.append(
				" GROUP BY MOVIMENTO_ESTOQUE_COTA.PRODUTO_EDICAO_ID	                                                                         ");
		...
		if (filtro.isDesconsiderarCotaAVista()) {
			...
			sqlTblReparte.append(
					" if(MEC.COTA_ID in (select cota_a_vista.ID from cota cota_a_vista where cota_a_vista.TIPO_COTA = 'A_VISTA'),0, SUM(	");
			sqlTblReparte.append(
					" 	COALESCE(if(tm.OPERACAO_ESTOQUE = 'SAIDA', MEC.qtde*-1, MEC.qtde),0)                                    ");
		...
		if (filtro.getIdBoxEncalhe() != null) {
			sqlTblReparte
					.append(" INNER JOIN CONTROLE_CONFERENCIA_ENCALHE_COTA CCEC ON (CCEC.COTA_ID = MEC.COTA_ID)   ");
		...
		sqlTblReparte.append(
				" INNER JOIN PRODUTO_EDICAO ON (PRODUTO_EDICAO.ID = CONFERENCIA_ENCALHE.PRODUTO_EDICAO_ID)                 ");
		sqlTblReparte.append(
				" INNER JOIN PRODUTO ON (PRODUTO_EDICAO.PRODUTO_ID = PRODUTO.ID)                                           ");
		sqlTblReparte.append(
				" INNER JOIN PRODUTO_FORNECEDOR ON (PRODUTO_FORNECEDOR.PRODUTO_ID = PRODUTO.ID)                            ");
		sqlTblReparte.append(
				" INNER JOIN FORNECEDOR ON (PRODUTO_FORNECEDOR.FORNECEDORES_ID = FORNECEDOR.ID)                            ");
		sqlTblReparte.append(
				" INNER JOIN PESSOA ON (PESSOA.ID = FORNECEDOR.JURIDICA_ID)                                                ");
		sqlTblReparte.append(
				" WHERE CCEC.DATA_OPERACAO BETWEEN :dataRecolhimentoInicial AND :dataRecolhimentoFinal                                           ");
		...
		sqlTblReparte.append(
				" INNER JOIN chamada_encalhe_cota cec on cec.CHAMADA_ENCALHE_ID = ce.id and cec.COTA_ID = c.id ");
		...
		sqlTblReparte.append(
				" INNER JOIN lancamento l on l.id = mec.LANCAMENTO_ID and l.id = cel.LANCAMENTO_ID and l.PRODUTO_EDICAO_ID = ce.PRODUTO_EDICAO_ID");

		sqlTblReparte.append(
				" LEFT OUTER JOIN PERIODO_LANCAMENTO_PARCIAL PLP ON PLP.ID = L.PERIODO_LANCAMENTO_PARCIAL_ID  ");
		...
		// sqlTblReparte.append(" AND case when l.periodo_lancamento_parcial_id
		// is not null then CE.DATA_RECOLHIMENTO BETWEEN
		// :dataRecolhimentoInicial AND :dataRecolhimentoFinal else 1 = 1 end
		// ");
		...
		if (filtro.getIdBoxEncalhe() != null) {
			...
			sqlTblReparte
					.append(" AND CCEC.DATA_OPERACAO BETWEEN :dataRecolhimentoInicial AND :dataRecolhimentoFinal ");
		}
		...
		sqlTblReparte.append(
				" GROUP BY MEC.PRODUTO_EDICAO_ID                                                                           ");

		final StringBuilder sql = new StringBuilder();

		sql.append(
				" FROM CONTROLE_CONFERENCIA_ENCALHE_COTA CCEC                                                               ");
		sql.append(
				" INNER JOIN BOX ON (CCEC.BOX_ID = BOX.ID) 																   ");
		sql.append(
				" INNER JOIN CONFERENCIA_ENCALHE ON (CONFERENCIA_ENCALHE.CONTROLE_CONFERENCIA_ENCALHE_COTA_ID = CCEC.ID)    ");
		sql.append(
				" INNER JOIN PRODUTO_EDICAO ON (PRODUTO_EDICAO.ID = CONFERENCIA_ENCALHE.PRODUTO_EDICAO_ID)                  ");
	...
	public List<ConsultaEncalheDTO> obterListaConsultaEncalhe(final FiltroConsultaEncalheDTO filtro) {
		...
		final RowMapper cotaRowMapper = new RowMapper() {
			...
			public Object mapRow(final ResultSet rs, final int arg1) throws SQLException {
				...
				dto.setRecolhimento(
						rs.getObject("diaRecolhimento") != null ? (Long) rs.getObject("diaRecolhimento") : null);
	...
	private String montarSqlEncalheDetalhe(final FiltroConsultaEncalheDetalheDTO filtro) {
		...
		sql.append(
				"	coalesce(if(PESSOA.TIPO='F',PESSOA.NOME,PESSOA.RAZAO_SOCIAL),PESSOA.NOME_FANTASIA) as nomeCota,    ");
	...
	private void montarParametrosDetalhesEncalhe(final FiltroConsultaEncalheDetalheDTO filtro,
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<ConsultaEncalheDetalheDTO> obterListaConsultaEncalheDetalhe(
		...
		final SQLQuery sqlquery = getSession().createSQLQuery(sql.toString())
				...
				.addScalar("numeroCota").addScalar("nomeCota").addScalar("observacao");
	...
	private void atribuirValoresAosParametrosEncalheDetalhe(final FiltroConsultaEncalheDetalheDTO filtro,
	...
	public ConsultaEncalheRodapeDTO obterValoresTotais(final FiltroConsultaEncalheDTO filtro) {
		...
		sql.append(
				"    (COALESCE(MOVIMENTO_ESTOQUE_COTA.PRECO_COM_DESCONTO,PRODUTO_EDICAO.PRECO_VENDA)) ), 0) as valorReparte,");

		sql.append("	coalesce(sum( ESTOQUE_PRODUTO_COTA.QTDE_DEVOLVIDA * ");

		sql.append(
				"    (COALESCE(MOVIMENTO_ESTOQUE_COTA.PRECO_COM_DESCONTO,PRODUTO_EDICAO.PRECO_VENDA)) ), 0) as valorEncalhe ");
		...
		sql.append(
				"	CHAMADA_ENCALHE.DATA_RECOLHIMENTO BETWEEN :dataRecolhimentoInicial AND :dataRecolhimentoFinal ");
		...
		final SQLQuery sqlquery = getSession().createSQLQuery(sql.toString())
				...
				.addScalar("valorReparte").addScalar("valorEncalhe");
	 ...
	 * @return String - hql
	 */
	private String getConsultaListaContagemDevolucao(final FiltroDigitacaoContagemDevolucaoDTO filtro,
			final boolean indBuscaTotalParcial, final boolean indBuscaQtd) {
		...
		if (indBuscaQtd) {
			...
			sql.append(
					"select count(quantidadeTotal) as quantidadeTotal, sum(valorTotalGeral) as valorTotalGeral from (");
		} else {
		...
		if (indBuscaQtd) {
			...
			/*
			 * sql.
			 * append(" (case when desconto_logistica_pe.ID is not null then 			"
			 * ); sql.
			 * append("  		desconto_logistica_pe.PERCENTUAL_DESCONTO            	"
			 * ); sql.
			 * append("  else												 		 	"
			 * ); sql.
			 * append(" 		case when desconto_logistica_prod.ID is not null then 	"
			 * ); sql.
			 * append(" 			 desconto_logistica_prod.PERCENTUAL_DESCONTO      	"
		...
		if (filtro.getIdFornecedor() != null) {
			...
			sql.append(" AND PROD_FORNEC.FORNECEDORES_ID NOT IN ( ").append(" SELECT ID FROM FORNECEDOR forn ")
					.append(" WHERE forn.SITUACAO_CADASTRO = 'ATIVO' ")
					.append(" AND forn.FORNECEDOR_UNIFICADOR_ID IS NOT NULL ").append(" UNION ")
	 ...
	 * @return Query
	 */
	private Query criarQueryComParametrosObterListaContagemDevolucao(final String hql,
			final FiltroDigitacaoContagemDevolucaoDTO filtro, final boolean indBuscaTotalParcial,
			final boolean indBuscaQtd) {

		Query query = null;

		if (indBuscaQtd) {

			query = getSession().createSQLQuery(hql).addScalar("quantidadeTotal", StandardBasicTypes.INTEGER)
					.addScalar("valorTotalGeral", StandardBasicTypes.BIG_DECIMAL);

		} else {

			query = getSession().createSQLQuery(hql).addScalar("codigoProduto", StandardBasicTypes.STRING)
	 ...
	 * 
	 * @see br.com.abril.nds.repository.MovimentoEstoqueCotaRepository#
	 * obterListaContagemDevolucao(br.com.abril.nds.dto.filtro.
	 * FiltroDigitacaoContagemDevolucaoDTO, boolean)
	 */
	@Override
	@SuppressWarnings("unchecked")
	public List<ContagemDevolucaoDTO> obterListaContagemDevolucao(final FiltroDigitacaoContagemDevolucaoDTO filtro,
			final boolean indBuscaTotalParcial) {

		final String hql = getConsultaListaContagemDevolucao(filtro, indBuscaTotalParcial, false);

		final Query query = criarQueryComParametrosObterListaContagemDevolucao(hql, filtro, indBuscaTotalParcial,
				false);
	 ...
	 * 
	 * @see br.com.abril.nds.repository.MovimentoEstoqueCotaRepository#
	 * obterValorTotalGeralContagemDevolucao(br.com.abril.nds.dto.filtro.
	 * FiltroDigitacaoContagemDevolucaoDTO,
	 * br.com.abril.nds.model.estoque.TipoMovimentoEstoque)
	 */
	@Override
	public BigDecimal obterValorTotalGeralContagemDevolucao(final FiltroDigitacaoContagemDevolucaoDTO filtro) {
		...
		sql.append(" SELECT ")
				...
				.append("	CONFERENCIA_ENCALHE.movimento_estoque_cota_id = MOV_ESTOQUE_COTA.ID ").append(" ) ")

				.append(" INNER JOIN PRODUTO_EDICAO ON (	")
				.append("	PRODUTO_EDICAO.ID = MOV_ESTOQUE_COTA.PRODUTO_EDICAO_ID ").append(" ) ")

				.append(" INNER JOIN PRODUTO PROD ON (			").append(" 	PRODUTO_EDICAO.PRODUTO_ID = PROD.ID	")
				.append(" ) 	")

				.append(" INNER JOIN PRODUTO_FORNECEDOR PROD_FORNEC ON (	")
				.append(" 	PROD.ID = PROD_FORNEC.PRODUTO_ID 				").append(" ) ")
				...
				.append("	SELECT ").append("	ITEM_CH_ENC_FORNECEDOR.PRODUTO_EDICAO_ID  ")

				.append("	from ").append("	ITEM_CHAMADA_ENCALHE_FORNECEDOR ITEM_CH_ENC_FORNECEDOR ")
	 ...
	 * 
	 * @see br.com.abril.nds.repository.MovimentoEstoqueCotaRepository#
	 * obterMovimentoCotaPorTipoMovimento(java.util.Date, java.lang.Long,
	 * br.com.abril.nds.model.estoque.GrupoMovimentoEstoque)
	 */
	@Override
	@SuppressWarnings("unchecked")
	public List<MovimentoEstoqueCota> obterMovimentoCotaPorTipoMovimento(final Date data, final Long idCota,
	 ...
	 * 
	 * @see br.com.abril.nds.repository.MovimentoEstoqueCotaRepository#
	 * obterQtdMovimentoCotaPorTipoMovimento(java.util.Date, java.lang.Long,
	 * br.com.abril.nds.model.estoque.GrupoMovimentoEstoque)
	 */
	@Override
	public Map<Long, BigInteger> obterQtdMovimentoCotaPorTipoMovimento(final Intervalo<Date> periodo, final Long idCota,
		...
		@SuppressWarnings("unchecked")
		final List<Object[]> listaResultados = query.list();
	...
	public List<MovimentoEstoqueCota> obterMovimentoCotaLancamentoPorTipoMovimento(final Date dataLancamento,
			final Long idCota, final List<GrupoMovimentoEstoque> gruposMovimentoEstoque) {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<MovimentoEstoqueCotaDTO> obterMovimentoCotasPorTipoMovimento(final Date data,
			final List<Integer> numCotas, final List<GrupoMovimentoEstoque> gruposMovimentoEstoque) {
		...
		hql.append(
				" 		 sum( case tipoMovimento.operacaoEstoque when 'ENTRADA' then (movimento.qtde) else (movimento.qtde * -1) end) as qtdeReparte ");
		...
		query.setParameterList("gruposMovimentoEstoqueEstorno",
	...
	private void gerarOrdenacaoDadosAbastecimento(final FiltroMapaAbastecimentoDTO filtro, final StringBuilder hql) {
		...
		if (coluna != null) {
			...
			case SEQUENCIA_MATRIZ:
				// nome = " codigoBox, roteiro.ordem, rota.ordem, rotaPDV.ORDEM,
				// sequenciaMatriz, nomeProduto ";
	...
	private void gerarOrdenacaoEntregador(final FiltroMapaAbastecimentoDTO filtro, final StringBuilder hql) {
		...
		final ColunaOrdenacaoEntregador coluna = ColunaOrdenacaoEntregador
				.getPorDescricao(filtro.getPaginacao().getSortColumn());
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<ProdutoAbastecimentoDTO> obterDetlhesDadosAbastecimento(final Long idBox,
	...
	private void gerarOrdenacaoDetalhesAbastecimento(final FiltroMapaAbastecimentoDTO filtro, final StringBuilder hql) {
		...
		final ColunaOrdenacaoDetalhes coluna = ColunaOrdenacaoDetalhes
				.getPorDescricao(filtro.getPaginacaoDetalhes().getSortColumn());
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<ProdutoAbastecimentoDTO> obterMapaAbastecimentoPorProdutoBoxRota(
			final FiltroMapaAbastecimentoDTO filtro) {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<ProdutoAbastecimentoDTO> obterMapaAbastecimentoPorProdutoBoxRoteiro(
			final FiltroMapaAbastecimentoDTO filtro) {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<ProdutoAbastecimentoDTO> obterMapaAbastecimentoPorCota(final FiltroMapaAbastecimentoDTO filtro) {
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<MovimentoEstoqueCota> obterMovimentoEstoqueCotaPor(
			final ParametrosRecolhimentoDistribuidor parametrosRecolhimentoDistribuidor, final Long idCota,
			final GrupoNotaFiscal grupoNotaFiscal, final List<GrupoMovimentoEstoque> listaGrupoMovimentoEstoques,
		...
		final int qtdeIteracao = GrupoNotaFiscal.NF_DEVOLUCAO_SIMBOLICA.equals(grupoNotaFiscal)
				|| GrupoNotaFiscal.NF_VENDA.equals(grupoNotaFiscal) ? 2 : 1;
		int i = 0;
		while (i < qtdeIteracao) {
			final StringBuilder sql = new StringBuilder("");

			sql.append(" SELECT DISTINCT movimentoEstoqueCota ").append(" FROM Lancamento lancamento ");
			...
			if (listaGrupoMovimentoEstoques != null && !listaGrupoMovimentoEstoques.isEmpty()) {
				sql.append(
						"   AND movimentoEstoqueCota.tipoMovimento.grupoMovimentoEstoque IN (:listaGrupoMoviementoEstoque) ");
			}
	...
	public Long obterQuantidadeProdutoEdicaoMovimentadoPorCota(final Long idCota, final Long idProdutoEdicao,
		...
		hql.append(
				" select sum(case when tipoMovimento.grupoMovimentoEstoque.operacaoEstoque = :entrada then movimentoEstoqueCota.qtde ");
		...
		if (lancamento != null) {
			hql.append(
					" and movimentoEstoqueCota.lancamento.dataRecolhimentoDistribuidor = :dataRecolhimentoDistribuidor ");
		}
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<MovimentoEstoqueCota> obterMovimentoEstoqueCotaPor(final Distribuidor distribuidor, final Long idCota,
			final List<GrupoMovimentoEstoque> listaGrupoMovimentoEstoques, final Intervalo<Date> periodo,
			final List<Long> listaFornecedores) {
		...
		sql.append(" WHERE movimentoEstoqueCota.status = :status ").append("   AND cota.id = :idCota ")
	...
	@Override
	public Long countObterMapaDeAbastecimentoPorEntregador(final FiltroMapaAbastecimentoDTO filtro) {
	...
	public List<ProdutoAbastecimentoDTO> obterMapaDeImpressaoPorEntregador(final FiltroMapaAbastecimentoDTO filtro) {
		...
		hql.append(
				"        coalesce(if(pessoaEnt.tipo = 'F',pessoaEnt.NOME, pessoaEnt.RAZAO_SOCIAL),pessoaEnt.nome_fantasia, '') as nomeEntregador, ");
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<ProdutoAbastecimentoDTO> obterMapaDeImpressaoPorEntregadorQuebrandoPorCota(
			final FiltroMapaAbastecimentoDTO filtro) {
	...
	public List<MovimentoEstoqueCota> obterPorLancamento(final Long idLancamento) {
		...
		final String hql = " select movimento from MovimentoEstoqueCota movimento "
				+ " join movimento.lancamento lancamento " + " where lancamento.id = :idLancamento ";
	...
	@Override
	public ValoresAplicados obterValoresAplicadosProdutoEdicao(final Integer numeroCota, final Long idProdutoEdicao,
			final Date dataOperacao) {
		...
		if (idProdutoEdicao == null) {
			throw new ValidacaoException(TipoMensagem.WARNING,
					"Produto Edição não informada ao obter Valores de Expedição.");
		}

		if (dataOperacao == null) {
			throw new ValidacaoException(TipoMensagem.WARNING,
					"Data de Operacao não informada ao obter Valores de Expedição.");
		}
	...
	public List<MovimentoEstoqueCota> obterMovimentoEstoqueCotaSemEstudoPor(final List<Long> idCota,
			final Intervalo<Date> periodo, final List<Long> listaIdFornecedores,
	...
	@Override
	public Date obterDataUltimaMovimentacaoReparteExpedida(final Integer numeroCota, final Long idProdutoEdicao) {

		final StringBuilder sql = new StringBuilder();

		sql.append(" select max(mec.DATA) as data ").append(" from ").append(" movimento_estoque_cota mec ")
				.append(" inner join ").append(" tipo_movimento tm on (tm.ID = mec.TIPO_MOVIMENTO_ID) ")
				.append(" inner join ").append(" lancamento on (lancamento.id = mec.LANCAMENTO_ID) ")
				.append(" inner join ").append(" cota on (cota.ID = mec.COTA_ID) ").append(" where ")
				.append(" tm.GRUPO_MOVIMENTO_ESTOQUE = :grupoMovimentoEstoque ")
				.append(" and mec.PRODUTO_EDICAO_ID = :idProdutoEdicao ").append(" and cota.NUMERO_COTA = :numeroCota ")
	...
	public void adicionarEmLoteDTO(final List<MovimentoEstoqueCotaDTO> movimentosEstoqueCota) {
		...
		session.doWork(new Work() {
			...
			public void execute(final Connection conn) throws SQLException {
				...
				final StringBuilder sqlQry = new StringBuilder().append("insert ")
				...
				final SqlParameterSource[] params = SqlParameterSourceUtils
						.createBatch(movimentosEstoqueCota.toArray());
	...
	public void updateByIdConsolidadoAndGrupos(Long idConsolidado, List<String> grupoMovimentoFinaceiros, String motivo,
			Long movimentoFinanceiroCota, StatusEstoqueFinanceiro statusEstoqueFinanceiro) {

		final StringBuilder sql = new StringBuilder();
		sql.append("UPDATE MOVIMENTO_ESTOQUE_COTA AS estoque ");
		sql.append(
				"join MOVIMENTO_FINANCEIRO_COTA movi on movi.id = estoque.MOVIMENTO_FINANCEIRO_COTA_ID AND movi.COTA_ID = estoque.COTA_ID ");
		...
		this.getSession().createSQLQuery(sql.toString()).setParameter("motivo", motivo)
				...
				.setParameterList("grupoMovimentoFinaceiros", grupoMovimentoFinaceiros).executeUpdate();
	...
	@Override
	public void updateById(Long id, ValoresAplicados valoresAplicados, BigInteger qtde) {
		final StringBuilder sql = new StringBuilder();
		...
		this.getSession().createQuery(sql.toString()).setParameter("precoVenda", valoresAplicados.getPrecoVenda())
				.setParameter("precoComDesconto", valoresAplicados.getPrecoComDesconto())
				.setParameter("valorDesconto", valoresAplicados.getValorDesconto()).setParameter("qtde", qtde)
	...
	public void updateById(Long id, MovimentoFinanceiroCota movimentoFinanceiroCota) {
		...
		this.getSession().createQuery(sql.toString()).setParameter("movimentoFinanceiroCota", movimentoFinanceiroCota)
				.setParameter("id", id).executeUpdate();
	}
	...
	public void updateById(Long id, StatusEstoqueFinanceiro statusEstoqueFinanceiro) {
		...
		this.getSession().createQuery(sql.toString()).setParameter("statusEstoqueFinanceiro", statusEstoqueFinanceiro)
				.setParameter("id", id).executeUpdate();
	}
	...
	public void updateByCotaAndDataOpAndGrupos(Long idCota, Date dataOperacao, List<String> grupoMovimentoFinaceiros,
		...
		final String sql = "UPDATE MOVIMENTO_ESTOQUE_COTA AS estoque " + "join MOVIMENTO_FINANCEIRO_COTA movi on "
				+ "movi.id = estoque.MOVIMENTO_FINANCEIRO_COTA_ID " + "join TIPO_MOVIMENTO tipo on "
				+ "movi.TIPO_MOVIMENTO_ID = tipo.id and tipo.tipo = 'FINANCEIRO' " + "SET estoque.MOTIVO = :motivo "
				+ ",estoque.MOVIMENTO_FINANCEIRO_COTA_ID = :movimentoFinanceiroCota "
				+ ",estoque.STATUS_ESTOQUE_FINANCEIRO = :statusEstoqueFinanceiro " + "where movi.COTA_ID = :idCota "
				+ "and tipo.GRUPO_MOVIMENTO_FINANCEIRO in (:grupoMovimentoFinaceiros)"
				+ "and movi.DATA = :dataOperacao ";

		this.getSession().createSQLQuery(sql).setParameter("motivo", motivo)
				.setParameter("movimentoFinanceiroCota", null)
				.setParameter("statusEstoqueFinanceiro", statusEstoqueFinanceiro.name()).setParameter("idCota", idCota)
				.setParameterList("grupoMovimentoFinaceiros", grupoMovimentoFinaceiros)
				.setParameter("dataOperacao", dataOperacao).executeUpdate();
	}
	...
	public List<MovimentoEstoqueCota> obterMovimentosComProdutoContaFirme(final Long idLancamento) {
		...
		hql.append(
				" select m from MovimentoEstoqueCota m join m.lancamento lancamento join m.tipoMovimento tipoMovimento  ");
	...
	public void atualizarPrecoProdutoExpedido(final Long idProdutoEdicao, final BigDecimal precoProduto) {
		...
		sql.append(" update movimento_estoque_cota mc ").append(" join lancamento l on mc.LANCAMENTO_ID = l.ID ")
		...
		query.setParameterList("statusLancamento",
				Arrays.asList(StatusLancamento.EXPEDIDO.name(), StatusLancamento.EM_BALANCEAMENTO_RECOLHIMENTO.name()));
		query.setParameter("idProdutoEdicao", idProdutoEdicao);
		query.setParameterList("statusRecebimentoReparte",
				Arrays.asList(GrupoMovimentoEstoque.RECEBIMENTO_REPARTE.name(),
						GrupoMovimentoEstoque.SOBRA_DE_COTA.name(), GrupoMovimentoEstoque.SOBRA_EM_COTA.name(),
						GrupoMovimentoEstoque.FALTA_DE_COTA.name(), GrupoMovimentoEstoque.FALTA_EM_COTA.name(),
	...
	public List<MovimentoEstoqueCota> obterMovimentosEstoqueCotaPorIds(List<Long> idsMEC) {
		...
		hql.append(" select mec from MovimentoEstoqueCota mec ").append(" where mec.id in (:idsMEC) ");
	...
	public BigDecimal obterValorExpedicaoCotaAVista(final Date dataMovimentacao, Boolean devolveEncalhe,
			boolean precoCapaHistoricoAlteracao) {
		...
		sql.append(
				"	(COALESCE(MEC.PRECO_VENDA, PE.PRECO_VENDA, 0) * SUM(CASE WHEN TM.OPERACAO_ESTOQUE=:opSaida THEN MEC.QTDE ELSE MEC.QTDE * -1 END)) AS total ");
		sql.append(" FROM MOVIMENTO_ESTOQUE_COTA MEC ");
		...
		if (devolveEncalhe != null) {
			...
			sql.append(
					" )       	AND (MEC.STATUS_ESTOQUE_FINANCEIRO is null OR MEC.STATUS_ESTOQUE_FINANCEIRO = :statusFinanceiroNaoProcessado)) ");
			sql.append("    )");
		} else {

			sql.append(" AND (");
			sql.append(
					"        c.TIPO_COTA = :tipoCotaAVista AND (MEC.STATUS_ESTOQUE_FINANCEIRO is null OR MEC.STATUS_ESTOQUE_FINANCEIRO = :statusFinanceiroNaoProcessado) ");
			sql.append("    )");
		}
		...
		sql.append(" HAVING ");
		sql.append(
				"    SUM(if(TM.OPERACAO_ESTOQUE=:opEntrada,MEC.QTDE,0) - if(TM.OPERACAO_ESTOQUE=:opSaida,MEC.QTDE,0 ))<>0 ");
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<AbastecimentoBoxCotaDTO> obterMapaDeImpressaoPorBoxVersusCotaQuebrandoPorCota(
			final FiltroMapaAbastecimentoDTO filtro) {
	...
	private void gerarFromWhereDadosAbastecimentoVersusBoxCota(final FiltroMapaAbastecimentoDTO filtro,
			final StringBuilder hql, final Map<String, Object> param, final List<String> statusLancamento) {
	...
	private StringBuilder obterQueryListaConsultaReparte(final FiltroConsultaEncalheDTO filtro,
			final boolean counting) {
		...
		final StringBuilder qtdeInformadaEncalhe = new StringBuilder(
				"select coalesce(sum(COALESCE(conf1.QTDE, 0)), 0) ");

		qtdeInformadaEncalhe.append(" from conferencia_encalhe conf1 ");
		qtdeInformadaEncalhe
				.append("  inner join chamada_encalhe_cota   cec on cec.id = conf1.chamada_encalhe_cota_id ");
		qtdeInformadaEncalhe.append(" inner join chamada_encalhe ce on ce.id = cec.CHAMADA_ENCALHE_ID ");
		qtdeInformadaEncalhe.append(
				" 		            where conf1.DATA BETWEEN :dataRecolhimentoInicial AND :dataRecolhimentoFinal ");
		...
		sql.append(
				" 			SELECT  coalesce(if(pessoa.tipo = 'F',pessoa.nome, pessoa.razao_social), pessoa.nome_fantasia, '') as nomeCota ,");
		...
		sql.append(
				" 		            SUM(COALESCE(if(tm.OPERACAO_ESTOQUE = 'SAIDA', MEC.qtde * - 1, MEC.qtde), 0)) AS REPARTE,");
		sql.append("( ( ").append(qtdeInformadaEncalhe).append(" ) - ( ").append(subSqlVendaProduto)
				.append(") ) as encalhe ");
		...
		sql.append(
				" 		        distinct PRODUTO_EDICAO.ID AS ID, b.id as boxid,b.tipo_box as tipobox, b.nome as boxnome,CCEC.COTA_ID as COTA_ID");
		sql.append(" 		    FROM");
		...
		sql.append(" 		    AND PRODUTO_EDICAO.ID = :idProdutoEdicao ");
		sql.append(
				" 		    GROUP BY  CCEC.COTA_ID) AS EDICAO_ENCALHADA ON (MEC.PRODUTO_EDICAO_ID = EDICAO_ENCALHADA.ID AND MEC.COTA_ID = EDICAO_ENCALHADA.COTA_ID and MEC.MOVIMENTO_ESTOQUE_COTA_FURO_ID is null)");
		...
		sql.append("  and tipobox = 'ENCALHE' "); // FILTRO PARA NAO PEGAR COTA
													// AUSENTE
		sql.append(" 		            AND MEC.LANCAMENTO_ID is not null");
	...
	private StringBuilder obterQueryListaConsultaBaseReparte(final FiltroConsultaEncalheDTO filtro,
			final boolean counting) {
	...
	public List<BigDecimal> obterDescontoPublicaoExpedida(Long produtoEdicaoId) {
		...
		hql.append(
				" where PRODUTO_EDICAO_ID = :produtoEdicaoId and STATUS_ESTOQUE_FINANCEIRO = 'FINANCEIRO_NAO_PROCESSADO' ");
	...
	public List<MovimentoEstoqueCota> obterMovimentoEstoqueCotaFinanceiroNaoProcessadoDePublicaoExpedida(
		...
		hql.append(
				" where produtoEdicao.id = :produtoEdicaoId and mec.statusEstoqueFinanceiro = 'FINANCEIRO_NAO_PROCESSADO' ");
