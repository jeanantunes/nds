...
public class FTFRepositoryImpl extends AbstractRepository implements FTFRepository {
	...
	public List<FTFEnvTipoRegistro02> obterResgistroTipo02(Long idNF, long idTipoNotaFiscal) {
		...
		.append(" ELSE ")
		.append("      LPAD(cast(pe.CODIGO_DE_BARRAS_CORPORATIVO as char), 18, ' ')  ")
	...
	public FTFEnvTipoRegistro05 obterRegistroTipo05(final Long idNF) {
		...
		sqlBuilder.append(" LPAD(replace(cast(round(sum(truncate(nfps.QUANTIDADE_COMERCIAL * VALOR_UNITARIO_COMERCIAL,2)),2) as CHAR), '.', ''), 15, '0') as valorParcela, ");
