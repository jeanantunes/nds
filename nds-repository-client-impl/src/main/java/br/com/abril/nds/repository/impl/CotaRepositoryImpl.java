...
public class CotaRepositoryImpl extends AbstractRepositoryModel<Cota, Long> implements CotaRepository {
    ...
    private void setFromWhereCotasSujeitasSuspensao(final StringBuilder sql) {
        ...
        sql.append(" FROM COTA COTA_ ")
           ...
           .append(" WHERE COTA_.SITUACAO_CADASTRO = :ativo ")
           .append(" AND COTA_.SUGERE_SUSPENSAO_DISTRIBUIDOR = true ")
           ...
           .append("	   ) ")
           .append("	   OR ")
           .append("	   (POLITICADISTRIB.VALOR_SUSPENSAO IS NOT NULL ")
           ...
           .append("         join NEGOCIACAO_COBRANCA_ORIGINARIA nco on ng.id = nco.NEGOCIACAO_ID ")
           ...
           .append("         	on cb.DIVIDA_ID = dv.ID and nco.COBRANCA_ID = cb.id")
    ...
    private void montarQueryReparteCotaAusente(final FiltroConsultaNotaEnvioDTO filtro, final StringBuilder sql,
        ...
        sql.append("   from " + "	        COTA cota_ " + "	    left outer join " + "	        BOX box1_  "
                + "	            on cota_.BOX_ID=box1_.ID  " + "	    inner join " + "	        MOVIMENTO_ESTOQUE_COTA mec  ignore index (FKBEB397FC4E3C0541459444C3,FK459444C3714FA744,NDX_MEC_NOTA_ENVIO,FK459444C345C07ACF) "
    ...
    private void montarQueryCotasComNotasEnvioNaoEmitidas(final FiltroConsultaNotaEnvioDTO filtro,
        ...
        sql.append(" INNER JOIN  ESTUDO e_ ON ec_.ESTUDO_ID=e_.ID ");
        sql.append(" INNER JOIN  LANCAMENTO lancamento_ ON e_.PRODUTO_EDICAO_ID=lancamento_.PRODUTO_EDICAO_ID AND e_.ID=lancamento_.ESTUDO_ID ");
        sql.append(" LEFT  JOIN  MOVIMENTO_ESTOQUE_COTA mec ignore index (FKBEB397FC4E3C0541459444C3,FK459444C3714FA744,NDX_MEC_NOTA_ENVIO,FK459444C345C07ACF) ON mec.LANCAMENTO_ID=lancamento_.id AND mec.COTA_ID=cota_.ID ");
...
//        sql.append("  rks.qtde qtdeRankingSegmento, "); Trocar para posição do ranking FATURAMENTO
	...
	public List<AbastecimentoBoxCotaDTO> obterCotasExpedicao(final Intervalo<Date> intervaloDataLancamento) {
        ...
        sql.append(" INNER JOIN  ESTUDO e_ ON ec_.ESTUDO_ID=e_.ID ");
        sql.append(" INNER JOIN  LANCAMENTO lancamento_ ON e_.PRODUTO_EDICAO_ID=lancamento_.PRODUTO_EDICAO_ID AND e_.ID=lancamento_.ESTUDO_ID ");
        sql.append(" LEFT  JOIN  MOVIMENTO_ESTOQUE_COTA mec ignore index (FKBEB397FC4E3C0541459444c3,FK459444C3714FA744) ON mec.LANCAMENTO_ID=lancamento_.id AND mec.COTA_ID=cota_.ID ");
        sql.append(" LEFT  JOIN  TIPO_MOVIMENTO tipo_mov ON tipo_mov.ID=mec.TIPO_MOVIMENTO_ID ");
        sql.append(" INNER JOIN  PRODUTO_EDICAO pe_ ON e_.PRODUTO_EDICAO_ID=pe_.ID ");
        sql.append(" INNER JOIN  PRODUTO p_ ON pe_.PRODUTO_ID=p_.ID ");
        sql.append(" INNER JOIN  PRODUTO_FORNECEDOR pf_ ON p_.ID=pf_.PRODUTO_ID ");
	...
	public List<CotaCouchDTO> getCotaRecolhimento(Date data) {
		...
		.append(" left outer join endereco on endereco.id = endpdv.endereco_id ")
		.append("  where ")
		.append(" 		ce.DATA_RECOLHIMENTO ='"+simpleDateFormat.format(data)+"'")
