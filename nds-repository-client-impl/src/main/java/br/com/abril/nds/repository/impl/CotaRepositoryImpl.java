...
public class CotaRepositoryImpl extends AbstractRepositoryModel<Cota, Long> implements CotaRepository {
	...
	private void setFromWhereCotasSujeitasSuspensao(StringBuilder sql) {
		...
		.append("			SELECT MAX(PIORDIVIDA.QTDEDIVIDASATIVAS) ")
		.append("			FROM (SELECT COUNT(ACUMDIVIDA.DIVIDA_ID) AS QTDEDIVIDASATIVAS, DIVI.COTA_ID AS ID_COTA ")
		.append("				  FROM HISTORICO_ACUMULO_DIVIDA ACUMDIVIDA ")
		.append("				  JOIN DIVIDA DIVI ON DIVI.ID = ACUMDIVIDA.DIVIDA_ID ")
		.append("				  WHERE ACUMDIVIDA.STATUS = :acumulaDivida GROUP BY ACUMDIVIDA.DIVIDA_ID) AS PIORDIVIDA ")
		.append("			WHERE PIORDIVIDA.ID_COTA = COTA_.ID) ")
		.append("	   ) ")
		.append("	   OR ")
		...
		.append("	   ) ")
		.append("	   OR ")
		.append("	   (POLITICADISTRIB.NUM_ACUMULO_DIVIDA IS NOT NULL ")
