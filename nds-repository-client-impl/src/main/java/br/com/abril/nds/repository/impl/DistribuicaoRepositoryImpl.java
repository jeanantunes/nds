...
public class DistribuicaoRepositoryImpl extends AbstractRepositoryModel<Lancamento, Long> implements DistribuicaoRepository {
	...
	public List<ProdutoDistribuicaoVO> obterMatrizDistribuicao(FiltroDistribuicaoDTO filtro) {
	 ...
	 sql.append(" select ")
        ...
        .append("       	((SELECT reparte FROM lancamento WHERE id = lanc.id and lanc.DATA_LCTO_DISTRIBUIDOR = :dataLanctoPrev) - ifnull(lanc.REPARTE_PROMOCIONAL, 0))       								      ") 
        .append("       WHEN plp.numero_periodo>1 THEN       																									  ") 
        .append("           (SELECT sum(lc.REPARTE) FROM lancamento lc WHERE lc.PRODUTO_EDICAO_ID = prodEdic.id AND lc.PERIODO_LANCAMENTO_PARCIAL_ID = plp.ID and lanc.DATA_LCTO_DISTRIBUIDOR = :dataLanctoPrev)      ") 
        .append("       ELSE          																															  ")   
        .append("           (SELECT sum(lc.REPARTE) FROM lancamento lc WHERE lc.PRODUTO_EDICAO_ID = prodEdic.id AND lanc.DATA_LCTO_DISTRIBUIDOR = :dataLanctoPrev)          										  ") 
        .append("       END          																																  ") 
        .append("   ELSE        																																	  ") 
