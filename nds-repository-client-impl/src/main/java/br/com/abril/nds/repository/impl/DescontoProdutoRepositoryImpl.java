...
public class DescontoProdutoRepositoryImpl extends AbstractRepositoryModel<DescontoProduto,Long> implements DescontoProdutoRepository {
	...
	public List<TipoDescontoProdutoDTO> buscarTipoDescontoProduto(FiltroTipoDescontoProdutoDTO filtro) {
		...
		hql.append(" SELECT dados.DESCONTO_ID as idTipoDesconto,p.CODIGO as codigoProduto, p.NOME as nomeProduto, "
				...
				+"	 	d.DATA_ALTERACAO as dataAlteracao, dados.QTDE_PROX_LANCAMENTOS_ORIGINAL as qtdeProxLcmt, dados.QTDE_COTAS as qtdeCotas");
	...
	private StringBuilder addQueryTipoDescontoProduto(StringBuilder sql) {
		...
		sql.append(" FROM (SELECT DESCONTO_ID, PRODUTO_ID, null as EDICAO_ID, hdp.USUARIO_ID, null as QTDE_PROX_LANCAMENTOS_ORIGINAL, null as QTDE_COTAS "  
					+"	 FROM HISTORICO_DESCONTO_PRODUTOS hdp "	
					+"	 UNION "	
					
					+"	 SELECT DESCONTO_ID, PRODUTO_ID, PRODUTO_EDICAO_ID as EDICAO_ID, hdcpe.USUARIO_ID, null as QTDE_PROX_LANCAMENTOS_ORIGINAL, count(COTA_ID) as QTDE_COTAS "
					+"	 FROM HISTORICO_DESCONTO_COTA_PRODUTO_EXCESSOES hdcpe " 
					+"	 where produto_id is not null " 
					+"	 group by DESCONTO_ID "	
					+"	 UNION "	
					
					+"	 SELECT DESCONTO_ID, PRODUTO_ID, PRODUTO_EDICAO_ID as EDICAO_ID, hdpe.USUARIO_ID, null as QTDE_PROX_LANCAMENTOS_ORIGINAL, null as QTDE_COTAS "
					+"	 FROM HISTORICO_DESCONTO_PRODUTO_EDICOES hdpe "	
					+"	 UNION "	
					
					+"	 SELECT dpl.DESCONTO_ID, PRODUTO_ID, null as EDICAO_ID, dpl.USUARIO_ID, QUANTIDADE_PROXIMOS_LANCAMENTOS_ORIGINAL as QTDE_PROX_LANCAMENTOS_ORIGINAL, " 
